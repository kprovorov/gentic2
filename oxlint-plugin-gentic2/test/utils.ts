...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "gentic2-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-gentic2", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "gentic2", specifier: pluginPath }],
