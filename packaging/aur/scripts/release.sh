...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="Gentic2-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit gentic2-icon.png
git config user.name 'gentic2-ci'
git config user.email 'gentic2-ci@users.noreply.github.com'
git add -A
