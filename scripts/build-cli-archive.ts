 ...
 * archive: the `g2` binary, the web client, the resource monitor, and a
 ...
 * archive stem):
 *
 *   g2 | g2.exe          the single-executable
...
export function cliArchiveStem(version: string, platform: BuildPlatform, arch: BuildArch): string {
  return `g2-${version}-${cliArchivePlatformKey(platform, arch)}`;
}
...
const MacSigningConfig = Config.all({
  identity: Config.String("GENTIC2_CLI_MAC_SIGN_IDENTITY").pipe(Config.option),
...
const signMacArchiveContents = Effect.fn("signMacArchiveContents")(function* (input: {
  ...
  const libraries = (yield* fs.readDirectory(input.contentDir, { recursive: true }))
    .filter(
      (entry) =>
        ...
        entry.endsWith("g2-resource-monitor"),
    )
  ...
  if (identity === "-") {
    yield* Effect.log("[cli-archive] Signed ad hoc (no GENTIC2_CLI_MAC_SIGN_IDENTITY).");
    return;
  }
  ...
  const notarizeZip = path.join(path.dirname(input.executablePath), ".notarize-g2.zip");
  yield* runCommand(
  ...
  yield* Effect.log("[cli-archive] Notarized g2.");
});
...
/** Signs g2.exe through the same Azure Trusted Signing setup the installer uses. */
const signWindowsExecutable = Effect.fn("signWindowsExecutable")(function* (
  ...
  yield* runCommand(
    ...
    "Invoke-TrustedSigning g2.exe",
  );
  yield* Effect.log("[cli-archive] Signed g2.exe (Azure Trusted Signing).");
});
...
const buildCliArchive = Effect.fn("buildCliArchive")(function* (input: {
  ...
  const executableName = input.platform === "win" ? "g2.exe" : "g2";
  // tsdown suffixes cross-built executables with their target (g2-darwin-x64);
  // a host build is plain g2. Prefer the exact target when both exist.
  const targetKey = `${input.platform === "mac" ? "darwin" : input.platform}-${input.arch}`;
  const targetExecutable = path.join(
    serverDir,
    "dist-exe",
    `g2-${targetKey}${input.platform === "win" ? ".exe" : ""}`,
  );
  ...
  yield* requireInput(path.join(webClient, "index.html"), "Run `vp run --filter g2 build` first.");
  yield* requireInput(
    resourceMonitorDir,
  ...
  const stageRoot = yield* fs.makeTempDirectoryScoped({ prefix: "g2-cli-archive-" });
...
).pipe(Command.withDescription("Package the g2 single-executable into a per-platform archive."));
