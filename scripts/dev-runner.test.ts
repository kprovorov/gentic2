...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit GENTIC2_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "GENTIC2_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "GENTIC2_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("forwards the reusable auth token to web dev and removes it for desktop", () =>
      Effect.gen(function* () {
        const input = {
          baseEnv: { GENTIC2_DEV_AUTH_TOKEN: "reusable-dev-auth-token-that-is-long-enough" },
          serverOffset: 0,
          webOffset: 0,
          g2Home: undefined,
          browser: undefined,
        ...
        assert.equal(web.GENTIC2_DEV_AUTH_TOKEN, input.baseEnv.GENTIC2_DEV_AUTH_TOKEN);
        assert.equal(desktop.GENTIC2_DEV_AUTH_TOKEN, undefined);
      }),
    );
    it.effect("leaves the shared home implicit and disables browser auto-open", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.GENTIC2_HOME, undefined);
        assert.equal(env.GENTIC2_NO_BROWSER, "1");
      }),
    );
    ...
    it.effect("allows browser auto-open to be explicitly enabled", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.GENTIC2_NO_BROWSER, "0");
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.GENTIC2_HOME, path.resolve("/tmp/custom-g2"));
        assert.equal(env.GENTIC2_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.GENTIC2_NO_BROWSER, "1");
        assert.equal(env.GENTIC2_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.GENTIC2_LOG_WS_EVENTS, "1");
        assert.equal(env.GENTIC2_HOST, "0.0.0.0");
    ...
    it.effect("strips inherited service-launcher context", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            G2_SERVICE_LAUNCHER_CONTEXT: '{"childVersion":"9.9.9"}',
            G2_BOOT_SERVICE_UNIT: "gentic2.service",
          },
          serverOffset: 0,
          webOffset: 0,
          g2Home: undefined,
        ...
        assert.equal(env.G2_SERVICE_LAUNCHER_CONTEXT, undefined);
        assert.equal(env.G2_BOOT_SERVICE_UNIT, undefined);
      }),
    );
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            GENTIC2_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          g2Home: undefined,
        ...
        assert.equal(env.GENTIC2_MODE, "web");
        assert.equal(env.GENTIC2_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.GENTIC2_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom g2Home when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
        ...
        assert.equal(env.GENTIC2_HOME, path.resolve("/tmp/my-g2"));
      }),
    );
    ...
    it.effect("pins desktop dev to a stable backend port and websocket url", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.GENTIC2_MODE, undefined);
        assert.equal(env.GENTIC2_NO_BROWSER, undefined);
        assert.equal(env.GENTIC2_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.GENTIC2_PORT, "13773");
        assert.equal(env.PORT, "5733");
      }),
    );
    ...
    for (const mode of ["dev", "dev:web"] as const) {
      it.effect(`leaves the client backend URLs unset in ${mode} mode`, () =>
        Effect.gen(function* () {
          ...
          assert.equal(env.GENTIC2_SINGLE_ORIGIN_DEV, "1");
        }),
      );
    }
    ...
    it.effect("clears the single-origin marker in dev:desktop mode", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: { GENTIC2_SINGLE_ORIGIN_DEV: "1" },
          serverOffset: 0,
          webOffset: 0,
          g2Home: undefined,
          browser: undefined,
        ...
        assert.equal(env.GENTIC2_SINGLE_ORIGIN_DEV, undefined);
    ...
    // --host configures the *backend* (GENTIC2_HOST). It must not become Vite's
  ...
  describe("devPortProbeHosts", () => {
    ...
    // actually bind, so --host/GENTIC2_HOST has to be probed as well.
    ...
    // Only the backend honours --host/GENTIC2_HOST. Vite reads HOST (set for
  ...
  describe("runDevRunnerWithInput", () => {
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { GENTIC2_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, ["GENTIC2_PORT_OFFSET", "GENTIC2_DEV_INSTANCE"]);
    ...
    // Base-dir precedence (--home-dir > worktree .g2 > ambient GENTIC2_HOME)
    ...
    it.effect("ignores an ambient GENTIC2_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { GENTIC2_HOME: "/home/user/.g2" },
          serverOffset: 0,
          webOffset: 0,
          g2Home: undefined,
          browser: undefined,
    ...
    // defaults: an explicit GENTIC2_BUNDLED_DEV (even "0") must pass through.
    describe("--share bundled dev default", () => {
          ...
          yield* runDevRunnerWithInput({
            ...
            Effect.provideService(
              ...
              input.ambientBundledDev === undefined
                ? {}
                : { GENTIC2_BUNDLED_DEV: input.ambientBundledDev },
            ),
          );
      ...
      it.effect("defaults GENTIC2_BUNDLED_DEV=1 for a shared run", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: undefined });
          assert.equal(env?.GENTIC2_BUNDLED_DEV, "1");
        }),
      );

      it.effect("keeps an explicit GENTIC2_BUNDLED_DEV=0 opt-out", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: "0" });
          assert.equal(env?.GENTIC2_BUNDLED_DEV, "0");
        }),
      );

      it.effect("leaves GENTIC2_BUNDLED_DEV unset without --share", () =>
        Effect.gen(function* () {
          let captured: Record<string, string | undefined> | undefined;
          const spawnerLayer = Layer.succeed(
            ChildProcessSpawner.ChildProcessSpawner,
          ...
          assert.equal(captured?.GENTIC2_BUNDLED_DEV, undefined);
        }),
      );
    });
    ...
    describe("g2 home precedence", () => {
      const makeWorktree = Effect.acquireRelease(
        Effect.sync(() => {
          const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "g2-devrunner-"));
      ...
      const spawnedHome = (input: {
        ...
        Effect.gen(function* () {
          ...
          yield* runDevRunnerWithInput({ ...devServerInput, g2Home: input.g2Home }).pipe(
            Effect.provide(Layer.mergeAll(emptyConfigLayer, netServiceLayer, spawnerLayer)),
            Effect.provideService(HostProcessPlatform, "linux"),
            Effect.provideService(HostProcessWorkingDirectory, input.cwd),
            Effect.provideService(
              HostProcessEnvironment,
              input.ambientHome === undefined ? {} : { GENTIC2_HOME: input.ambientHome },
            ),
          );

          return captured?.GENTIC2_HOME;
        });
      ...
      it.effect("prefers an explicit --home-dir over the worktree default", () =>
        Effect.gen(function* () {
          ...
          const home = yield* spawnedHome({
            g2Home: "/tmp/explicit-home",
            cwd: root,
            ambientHome: "/home/user/.g2",
          });
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".g2"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the worktree .g2 over an ambient GENTIC2_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("falls back to an ambient GENTIC2_HOME outside a worktree", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const home = yield* spawnedHome({
            g2Home: undefined,
          ...
          assert.equal(home, path.resolve("/home/user/.g2"));
        }),
      );
