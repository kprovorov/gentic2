...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.GENTIC2_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.GENTIC2_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.GENTIC2_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.GENTIC2_RELAY_URL).toBeUndefined();
    expect(env.VITE_GENTIC2_RELAY_URL).toBeUndefined();
    expect(env.GENTIC2_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.GENTIC2_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.GENTIC2_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.GENTIC2_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.GENTIC2_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.GENTIC2_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "GENTIC2_CLERK_PUBLISHABLE_KEY=pk_root\nGENTIC2_CLERK_JWT_TEMPLATE=template_root\nGENTIC2_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nGENTIC2_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "GENTIC2_CLERK_PUBLISHABLE_KEY=pk_local\nGENTIC2_CLERK_JWT_TEMPLATE=template_local\nGENTIC2_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nGENTIC2_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).GENTIC2_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          GENTIC2_CLERK_PUBLISHABLE_KEY: "pk_ci",
          GENTIC2_CLERK_JWT_TEMPLATE: "template_ci",
          GENTIC2_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          GENTIC2_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      GENTIC2_RELAY_URL: "https://ci.example.test",
      VITE_GENTIC2_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        GENTIC2_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_GENTIC2_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          GENTIC2_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          GENTIC2_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          GENTIC2_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          GENTIC2_RELAY_URL: "https://relay.example.test",
          GENTIC2_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          GENTIC2_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          GENTIC2_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      GENTIC2_RELAY_URL: "https://relay.example.test",
      VITE_GENTIC2_RELAY_URL: "https://relay.example.test",
      GENTIC2_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      GENTIC2_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      GENTIC2_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "gentic2-public-config-"));
