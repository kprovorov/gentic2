...
/** A tar.gz laid out like build-cli-archive.ts writes, with a stub `g2` that echoes its args. */
const makeFakeArchives = Effect.fn("test.makeFakeArchives")(function* () {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "g2-npm-packages-test-" });
  const archivesDir = path.join(root, "archives");
  yield* fs.makeDirectory(archivesDir);
  for (const key of KEYS) {
    const stem = `g2-${VERSION}-${key}`;
    ...
    yield* fs.writeFileString(
      path.join(contentDir, "g2"),
      `#!/bin/sh\necho "stub ${key} $*"\nexit 7\n`,
    );
    yield* fs.chmod(path.join(contentDir, "g2"), 0o755);
  ...
  it.effect("builds platform packages and a launcher that execs the installed one", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        ["@gentic2/g2-darwin-arm64", "@gentic2/g2-linux-x64", "g2"],
      );
      for (const output of outputs) {
        assert.isTrue(yield* fs.exists(output.tarball), output.tarball);
      }

      const linuxDir = path.join(fixture.outputDir, "@gentic2/g2-linux-x64");
      const linuxManifest = yield* decodeManifest(
        yield* fs.readFileString(path.join(linuxDir, "package.json")),
      );
      assert.equal(linuxManifest.name, "@gentic2/g2-linux-x64");
      ...
      assert.deepStrictEqual(outputEntries.sort(), ["@gentic2", "g2", "g2.tgz"]);
      ...
      const listing = yield* run(
        "tar",
        ["-tzvf", path.join(fixture.outputDir, "@gentic2/g2-linux-x64.tgz")],
      ...
      assert.isTrue(
        lines.some((line) => /^-rwxr-xr-x .* package\/g2$/.test(line)),
        listing.stdout,
      );
      ...
      if (KEYS.some((key) => key === `${hostPlatform}-${hostArch}`)) {
        const passthrough = yield* run(process.execPath, ["bin/g2.js", "serve", "--port", "1234"], {
          cwd: launcherDir,
          env,
        });
        assert.equal(
        ...
        const unpack = yield* run(
          "tar",
          ["-xf", path.join(fixture.outputDir, "g2.tgz"), "-C", installedLauncher],
          {
            cwd: fixture.root,
          },
        );
      ...
      const unsupported = yield* run(process.execPath, ["bin/g2.js", "--version"], {
        cwd: launcherDir,
      ...
      assert.include(unsupported.stderr, "https://github.com/kprovorov/gentic2/releases");
    }),
  );
});
