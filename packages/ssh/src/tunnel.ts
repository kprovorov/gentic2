...
} from "@gentic2/shared/httpReadiness";
import { cliReleaseDownloadBaseUrl } from "@gentic2/shared/cliRelease";
import * as NetService from "@gentic2/shared/Net";
import { extractJsonObject, fromLenientJson } from "@gentic2/shared/schemaJson";
...
export interface RemoteG2RunnerOptions {
  /**
...
export interface SshEnvironmentManagerOptions {
  readonly resolveCliRunner?: Effect.Effect<RemoteG2RunnerOptions>;
}
...
function isNodeScriptRunner(runner: RemoteG2RunnerOptions | undefined): boolean {
  return Boolean(runner?.nodeScriptPath?.trim());
}

function sshRunnerLogFields(runner: RemoteG2RunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  G2_NODE_ENGINE_RANGE=@@G2_NODE_ENGINE_RANGE@@
  if [ -z "$G2_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$G2_NODE_ENGINE_RANGE" <<'NODE'
@@G2_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for G2_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$G2_NODE_BIN/node" ]; then
        PATH="$G2_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@G2_NODE_ENV_SCRIPT@@
G2_NODE_SCRIPT_PATH=@@G2_NODE_SCRIPT_PATH@@
if [ -n "$G2_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$G2_NODE_SCRIPT_PATH" "$@"
fi
G2_ARCHIVE_VERSION=@@G2_ARCHIVE_VERSION@@
if [ -z "$G2_ARCHIVE_VERSION" ]; then
  printf 'No g2 release version was provided for the remote runtime.\\n' >&2
  exit 1
fi
# Self-contained release archive: no Node, npm, or compiler on the remote.
# Unpacked into the pinned-runtime layout so \`g2 service install\` reuses it.
G2_RELEASE_BASE_URL=@@G2_RELEASE_BASE_URL@@
G2_RUNTIME_DIR="$HOME/.g2/runtime/versions/$G2_ARCHIVE_VERSION"
g2_runtime_ready() {
  [ -x "$G2_RUNTIME_DIR/g2" ] && [ "$(cat "$G2_RUNTIME_DIR/.install-complete" 2>/dev/null)" = "$G2_ARCHIVE_VERSION" ]
}
if ! g2_runtime_ready; then
  mkdir -p "$HOME/.g2/runtime/versions"
  ...
  # under it.
  G2_LOCK="$HOME/.g2/runtime/versions/.$G2_ARCHIVE_VERSION.install.lock"
  ...
  G2_LOCK_WAITED=0
  G2_LOCK_UNOWNED=0
  while ! mkdir "$G2_LOCK" 2>/dev/null; do
    G2_LOCK_OWNER="$(cat "$G2_LOCK/pid" 2>/dev/null || true)"
    if [ -n "$G2_LOCK_OWNER" ]; then
      G2_LOCK_UNOWNED=0
      if ! kill -0 "$G2_LOCK_OWNER" 2>/dev/null; then
        rm -rf "$G2_LOCK"
        continue
      fi
    else
      G2_LOCK_UNOWNED=$((G2_LOCK_UNOWNED + 1))
      if [ "$G2_LOCK_UNOWNED" -ge 5 ]; then
        rm -rf "$G2_LOCK"
        continue
      fi
    fi
    if [ "$G2_LOCK_WAITED" -ge @@G2_ARCHIVE_LOCK_WAIT_SECONDS@@ ]; then
      printf 'Another g2 %s installation has held %s for too long.\\n' "$G2_ARCHIVE_VERSION" "$G2_LOCK" >&2
      exit 1
    fi
    sleep 1
    G2_LOCK_WAITED=$((G2_LOCK_WAITED + 1))
  done
  printf '%s\\n' "$$" > "$G2_LOCK/pid.tmp" && mv "$G2_LOCK/pid.tmp" "$G2_LOCK/pid"
  trap 'rm -rf "$G2_LOCK"' EXIT
fi
if ! g2_runtime_ready; then
  case "$(uname -s)" in
    Darwin) G2_PLATFORM="darwin" ;;
    Linux) G2_PLATFORM="linux" ;;
    *) printf 'Remote host %s has no g2 release archive.\\n' "$(uname -s)" >&2; exit 1 ;;
  esac
  case "$(uname -m)" in
    arm64 | aarch64) G2_ARCH="arm64" ;;
    x86_64 | amd64) G2_ARCH="x64" ;;
    *) printf 'Remote host %s has no g2 release archive.\\n' "$(uname -m)" >&2; exit 1 ;;
  esac
  G2_ARCHIVE="g2-$G2_ARCHIVE_VERSION-$G2_PLATFORM-$G2_ARCH.tar.gz"
  G2_STAGING="$(mktemp -d "$HOME/.g2/runtime/versions/.staging-XXXXXX")"
  trap 'rm -rf "$G2_STAGING" "$G2_LOCK"' EXIT
  g2_fetch() {
  ...
  g2_fetch "$G2_RELEASE_BASE_URL/v$G2_ARCHIVE_VERSION/SHA256SUMS" "$G2_STAGING/SHA256SUMS" @@G2_ARCHIVE_CHECKSUMS_SECONDS@@
  g2_fetch "$G2_RELEASE_BASE_URL/v$G2_ARCHIVE_VERSION/$G2_ARCHIVE" "$G2_STAGING/$G2_ARCHIVE" @@G2_ARCHIVE_DOWNLOAD_SECONDS@@
  G2_EXPECTED="$(grep " \\*\\{0,1\\}$G2_ARCHIVE$" "$G2_STAGING/SHA256SUMS" | cut -d' ' -f1)"
  if command -v sha256sum >/dev/null 2>&1; then
    G2_ACTUAL="$(sha256sum "$G2_STAGING/$G2_ARCHIVE" | cut -d' ' -f1)"
  else
    G2_ACTUAL="$(shasum -a 256 "$G2_STAGING/$G2_ARCHIVE" | cut -d' ' -f1)"
  fi
  if [ -z "$G2_EXPECTED" ] || [ "$G2_ACTUAL" != "$G2_EXPECTED" ]; then
    printf 'Checksum mismatch for %s.\\n' "$G2_ARCHIVE" >&2; exit 1
  fi
  tar -xzf "$G2_STAGING/$G2_ARCHIVE" -C "$G2_STAGING" --strip-components=1
  rm -f "$G2_STAGING/$G2_ARCHIVE" "$G2_STAGING/SHA256SUMS"
  ...
  if ! "$G2_STAGING/g2" --version >/dev/null 2>&1; then
    printf 'The g2 %s executable does not run on this host.\\n' "$G2_ARCHIVE_VERSION" >&2; exit 1
  fi
  printf '%s\\n' "$G2_ARCHIVE_VERSION" > "$G2_STAGING/.install-complete"
  rm -rf "$G2_RUNTIME_DIR"
  mv "$G2_STAGING" "$G2_RUNTIME_DIR"
fi
if [ -n "\${G2_LOCK:-}" ]; then
  rm -rf "$G2_LOCK"
  trap - EXIT
fi
exec "$G2_RUNTIME_DIR/g2" "$@"
`;

const REMOTE_LAUNCH_SCRIPT = `set -eu
@@G2_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.g2/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.g2"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-g2.sh"
RUNNER_NEXT="$STATE_DIR/run-g2.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
G2_ARCHIVE_MODE=@@G2_ARCHIVE_MODE@@
if [ "$G2_ARCHIVE_MODE" = "1" ]; then
...
pick_port() {
  if [ "$G2_ARCHIVE_MODE" = "1" ]; then
    "$RUNNER_FILE" __ssh-helper pick-port "$PORT_FILE" "@@G2_DEFAULT_REMOTE_PORT@@" "@@G2_REMOTE_PORT_SCAN_WINDOW@@"
    return
  fi
  node - "$PORT_FILE" "@@G2_DEFAULT_REMOTE_PORT@@" "@@G2_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@G2_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  if [ "$G2_ARCHIVE_MODE" = "1" ]; then
    "$RUNNER_FILE" __ssh-helper wait-ready "$REMOTE_PORT" "$1" "@@G2_READY_PROBE_TIMEOUT_MS@@"
    return
  fi
  node - "$REMOTE_PORT" "$1" "@@G2_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@G2_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@G2_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@G2_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@G2_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env GENTIC2_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@G2_READY_TIMEOUT_MS@@"; then
    printf 'Remote G2 server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
STATE_DIR="$HOME/.g2/ssh-launch/@@G2_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.g2"
RUNNER_FILE="$STATE_DIR/run-g2.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
if [ "$REMOTE_MANAGED" != "external" ] && [ -n "$REMOTE_PID" ] && kill -0 "$REMOTE_PID" 2>/dev/null; then
  ...
  if kill -0 "$REMOTE_PID" 2>/dev/null; then
    printf 'Remote G2 server with PID %s did not stop within 2 seconds. Its ownership files were kept.\\n' "$REMOTE_PID" >&2
    exit 1
  fi
fi
rm -f "$PID_FILE" "$PORT_FILE" "$MANAGED_FILE"
...
export class SshInvalidArchiveVersionError extends Schema.TaggedError<SshInvalidArchiveVersionError>()(
  ...
  override get message(): string {
    return `'${this.archiveVersion}' is not an exact g2 version and cannot name a runtime directory.`;
  }
}
...
export class SshMissingRunnerError extends Schema.TaggedError<SshMissingRunnerError>()(
  ...
  override get message(): string {
    return "A remote g2 runner needs an archive version or a node script path.";
  }
}

export function buildRemoteG2RunnerScript(input?: RemoteG2RunnerOptions): string {
  ...
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      G2_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      G2_ARCHIVE_VERSION: shellSingleQuote(archiveVersion),
      G2_RELEASE_BASE_URL: shellSingleQuote(releaseBaseUrl),
      G2_ARCHIVE_LOCK_WAIT_SECONDS: String(REMOTE_ARCHIVE_LOCK_WAIT_SECONDS),
      G2_ARCHIVE_DOWNLOAD_SECONDS: String(REMOTE_ARCHIVE_DOWNLOAD_SECONDS),
      G2_ARCHIVE_CHECKSUMS_SECONDS: String(REMOTE_ARCHIVE_CHECKSUMS_SECONDS),
      G2_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemoteG2RunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      G2_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      G2_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteG2RunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    G2_ARCHIVE_MODE: isNodeScriptRunner(input) ? "0" : "1",
    G2_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    G2_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteG2RunnerScript(input)),
    G2_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    G2_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    G2_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    G2_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    G2_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    G2_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    G2_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteG2RunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    G2_STATE_KEY: remoteStateKey(target),
    G2_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteG2RunnerScript(input)),
  });
}
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteG2RunnerOptions;
...
>()("@gentic2/ssh/tunnel/SshEnvironmentManager") {
