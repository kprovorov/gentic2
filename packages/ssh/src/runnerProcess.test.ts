...
import { buildRemoteStopScript, buildRemoteG2RunnerScript } from "./tunnel.ts";
  ...
  () => {
    it.live("keeps the server PID and graceful shutdown through the node-script runner", () =>
      Effect.gen(function* () {
        ...
        const fixture = yield* fs.makeTempDirectoryScoped({ prefix: "g2-runner-" });
...
server.listen(Number(process.env.G2_TEST_PORT ?? 0), "127.0.0.1", () => {
          ...
          Effect.gen(function* () {
            const child = yield* spawner.spawn(
              ChildProcess.make("/bin/sh", ["-s", "--", "serve", "a path with spaces"], {
                ...
                env: {
                  PATH: bin,
                  G2_TEST_PORT: String(port),
                },
                detached: false,
                stdin: Stream.make(
                  new TextEncoder().encode(buildRemoteG2RunnerScript({ nodeScriptPath: cliPath })),
                ),
              }),
            );
    ...
    it.live.each(["graceful", "timeout", "external"] as const)(
      ...
      (mode) =>
        Effect.gen(function* () {
          ...
          const path = yield* Path.Path;
          const spawner = yield* ChildProcessSpawner.ChildProcessSpawner;
          const fixture = yield* fs.makeTempDirectoryScoped({ prefix: "g2-stop-" });
