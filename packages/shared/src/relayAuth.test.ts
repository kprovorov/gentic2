...
describe("Clerk relay auth", () => {
  it("derives a custom Frontend API hostname from a Clerk publishable key", () => {
    expect(
      clerkFrontendApiHostnameFromPublishableKey(clerkPublishableKey("clerk.gentic2.com")),
    ).toBe("clerk.gentic2.com");
    expect(clerkFrontendApiUrlFromPublishableKey(clerkPublishableKey("clerk.gentic2.com"))).toBe(
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.gentic2.com/path";
