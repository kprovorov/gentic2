/**
 * `dist/bin.mjs` of the `g2` npm package: the entry point boot-service
 ...
 * executable in the sibling `@gentic2/g2-<platform>-<arch>` package.
 *
...
const executable = join(dirname(require.resolve("@gentic2/g2-" + process.platform + "-" + process.arch + "/package.json")), executableName);
...
const fail = (error) => {
  if (!error) return;
  process.stderr.write("g2: " + error.message + "\\n");
  child.kill("SIGTERM");
  process.exitCode = 1;
};
if (ipc) {
