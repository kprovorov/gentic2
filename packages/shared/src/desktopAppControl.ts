 ...
 * The state directory is hashed so custom Gentic2 homes cannot exceed Unix socket
 * path limits.
 */
export function resolveDesktopAppControlAddress(input: {
  ...
  if (input.platform === "win32") {
    return {
      address: `\\\\.\\pipe\\gentic2-app-${stateHash}`,
      directory: null,
    };
  }
  ...
  const directory = input.joinPath(input.tempDir, `gentic2-${userKey}`);
  return {
