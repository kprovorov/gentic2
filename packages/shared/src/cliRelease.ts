...
export function cliArchiveFileName(version: string, platformKey: CliArchivePlatformKey): string {
  return `g2-${version}-${platformKey}.${platformKey.startsWith("win32") ? "zip" : "tar.gz"}`;
}
