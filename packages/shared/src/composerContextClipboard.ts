...
export function encodeComposerContextClipboardHtml(
  ...
  if (html !== undefined)
    return `<div data-g2-context-fragment="${encodeURIComponent(fragment)}">${html}</div>`;
  const escaped = text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  return `<pre data-g2-context-fragment="${encodeURIComponent(fragment)}">${escaped}</pre>`;
}
...
export function decodeComposerContextClipboardHtml(
  ...
  const encoded = /data-g2-context-fragment=["']([^"']+)["']/.exec(html)?.[1];
