import type { ProjectId, ProjectScript, ServerSettings } from "@gentic2/contracts";
...
export function projectScriptRuntimeEnv(
  ...
  const env: Record<string, string> = {
    GENTIC2_PROJECT_ROOT: input.project.cwd,
  };
  if (input.worktreePath) {
    env.GENTIC2_WORKTREE_PATH = input.worktreePath;
  }
  if (input.extraEnv) {
