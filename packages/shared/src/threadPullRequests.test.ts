...
describe("threadPullRequestKeysEqual", () => {
  ...
  it("ignores host and repository case", () => {
    expect(
      threadPullRequestKeysEqual(
        { host: "GitHub.com", repository: "kprovorov/gentic2", number: 1 },
        { host: "github.com", repository: "kprovorov/gentic2", number: 1 },
      ),
    ).toBe(true);
    expect(
      threadPullRequestKeysEqual(
        { host: "github.com", repository: "kprovorov/gentic2", number: 1 },
        { host: "gitlab.com", repository: "kprovorov/gentic2", number: 1 },
      ),
    ).toBe(false);
  });
});
...
describe("resolveThreadCurrentPullRequest", () => {
  ...
  it("reports a stack when several links are open and puts the highest layer on top", () => {
    const stack = {
      ...
      id: "s1",
      number: 1,
      url: "https://github.com/kprovorov/gentic2/stacks/1",
      base: "main",
      layers: [
  ...
  it.each([
    {
      ...
      url: "https://github.enterprise.test/kprovorov/gentic2/pull/7",
    },
...
describe("threadPullRequestSearchTerms", () => {
  it("includes completed and unsynced links but excludes dismissed links", () => {
    ...
    expect(terms).toContain("kprovorov/gentic2#12");
    expect(terms).toContain("https://github.com/kprovorov/gentic2/pull/12");
...
it("searches the legacy projection when old environments decode to an empty links list", () => {
  const linkedPullRequest = {
    ...
    repository: "kprovorov/gentic2",
    number: 12,
    url: "https://github.com/kprovorov/gentic2/pull/12",
  };
