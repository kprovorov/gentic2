...
import { ThreadId } from "@gentic2/contracts";
...
describe("resolveWorkEntryToolPresentation", () => {
  it.each([
    "mcp__gentic2__preview_click",
    "mcp__gentic2__preview_click",
    "mcp__gentic2__preview_click",
    "Gentic2.preview_click",
    "gentic2 · preview_click completed",
    "gentic2/preview_click",
    "preview_click",
  ...
  it("labels device tools with the device icon", () => {
    expect(
      resolveWorkEntryToolPresentation({
        label: "mcp__gentic2__device_open",
        toolLifecycleStatus: "completed",
      }),
    ).toEqual({ displayName: "Opened a device in the Device panel", icon: "device" });
    expect(resolveWorkEntryToolPresentation({ label: "gentic2 · device_screenshot" })).toEqual({
  ...
  it("uses structured MCP identity when the provider supplies a custom title", () => {
    expect(
      resolveWorkEntryToolPresentation({
        ...
        toolData: { server: "gentic2", tool: "preview_snapshot", result: { title: "Example" } },
      }),
  ...
  it("uses the summary's state only when the provider omitted a lifecycle status", () => {
    const entry = { label: "Gentic2.preview_click" };
  ...
  it.each([
    ...
    ["g2_thread_read", "Reading a G2 thread", "Read a G2 thread"],
    ["g2_thread_send", "Sending to a G2 thread", "Sent to a G2 thread"],
    [
      "g2_worktree_handoff",
    ...
    const entry = { label: `gentic2.${tool}` };
    expect(
  ...
  it("keeps G2 branding for non-browser tools and falls back to the original tool label", () => {
    expect(
      resolveWorkEntryToolPresentation({
        label: "mcp__gentic2__task_status",
        toolTitle: "Check the child task",
      }),
    ).toEqual({ displayName: "Getting delegated task status", icon: "gentic2" });
  });

  it("does not brand unknown tools or another server's matching tool name", () => {
    for (const label of [
      "mcp__github__preview_click",
      "gentic2.unknown_tool",
      "gentic2.toString",
      "Search files",
    ]) {
...
describe("browser group summaries", () => {
  const browserEntry: WorkLogPresentationEntry = {
    label: "MCP tool call",
    toolData: { server: "gentic2", tool: "preview_click" },
  ...
  it("preserves first-seen action ordering alongside non-browser tools", () => {
    expect(
      summarizeToolGroup([
        ...
        {
          ...browserEntry,
          toolData: { server: "gentic2", tool: "task_status" },
        },
      ]),
      ...
      summarizeToolGroup([
        {
          ...
          toolData: { toolName: "mcp__gentic2__preview_evaluate" },
        },
      ]),
...
describe("resolveViewedImageAsset", () => {
  ...
  it("serves g2 attachment paths in place like any other host path", () => {
    const path = "/Users/demo/.g2/dev/attachments/11111111-1111-4111-8111-111111111111.png";
...
describe("pull request tool presentation", () => {
  it.each([
    "mcp__gentic2__link_pull_request",
    "mcp__gentic2__link_pull_request",
    "Gentic2 · link_pull_request",
    "gentic2/link_pull_request",
    "link_pull_request",
  ...
  it("recognizes unlink targets supplied as repository and number", () => {
    expect(
      resolveWorkEntryToolPresentation({
        ...
        toolLifecycleStatus: "completed",
        toolData: {
          toolName: "mcp__gentic2__unlink_pull_request",
  ...
  it("summarizes native PR work separately from ordinary tools and integration metadata", () => {
    const link: WorkLogPresentationEntry = {
      label: "Gentic2 · link_pull_request",
      tone: "tool",
      itemType: "mcp_tool_call",
      toolLifecycleStatus: "completed",
      toolSource: { key: "gentic2", name: "Gentic2", kind: "integration" },
    };
    const list: WorkLogPresentationEntry = {
      ...link,
      label: "Gentic2 · list_thread_pull_requests",
    };
    expect(summarizeToolGroup([link, link, list])).toBe(
      "Linked 2 pull requests and checked linked pull requests",
    );
    expect(summarizeToolGroup([{ ...link, label: "Gentic2 · unlink_pull_request" }])).toBe(
...
describe("device group summaries", () => {
  ...
  it("recognizes Claude tool names and preserves screenshot previews", () => {
    const entry = {
      ...
      toolData: { toolName: "mcp__gentic2__device_screenshot" },
  ...
  it("does not classify another server's tools as G2 device controls", () => {
    expect(
      summarizeToolGroup([
        {
          ...deviceEntry("device_open"),
