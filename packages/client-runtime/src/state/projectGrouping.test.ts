...
describe("buildProjectGroups", () => {
  it("preserves every physical clone as a selectable member in repository modes", () => {
    const projects = [
      makeProject("gentic2", "/work/gentic2"),
      makeProject("gentic2-2", "/work/gentic2-2"),
      makeProject("gentic2-3", "/work/gentic2-3"),
    ];
  ...
  it("uses a shared custom title as the repository group's label", () => {
    const projects = [
      makeProject("first", "/work/gentic2", { title: "Custom project" }),
      makeProject("second", "/work/gentic2-2", { title: "Custom project" }),
    ];
  ...
  it("keeps the repository label when shared titles match its repository name", () => {
    const projects = [
      makeProject("first", "/work/gentic2", { title: "gentic2" }),
      makeProject("second", "/work/gentic2-2", { title: "gentic2" }),
    ];
  ...
  it("keeps physical clones in separate groups when requested", () => {
    ...
    expect(groups.map((group) => group.label)).toEqual(["gentic2", "gentic2-2", "gentic2-3"]);
  });

  it("applies a physical-project override without dropping its siblings", () => {
    const first = makeProject("gentic2", "/work/gentic2");
    const second = makeProject("gentic2-2", "/work/gentic2-2");
    const third = makeProject("gentic2-3", "/work/gentic2-3");
  ...
  it("dedupes stale registrations at one physical path using the freshest project", () => {
    const stale = makeProject("stale", "/work/gentic2", {
      repositoryIdentity: null,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const fresh = makeProject("fresh", "/work/gentic2/", {
  ...
  it("uses repository identity from a duplicate registration when the winner lacks it", () => {
    const identified = makeProject("identified", "/work/gentic2", {
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshUnidentified = makeProject("fresh", "/work/gentic2/", {
      repositoryIdentity: null,
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const sibling = makeProject("sibling", "/work/gentic2-2");
  ...
  it("uses the freshest winner's repository identity when stale duplicates disagree", () => {
    const staleIdentity = {
      ...repositoryIdentity,
      canonicalKey: "github.com/gentic2/old-repository",
      name: "old-repository",
  ...
  it("uses the freshest identity-bearing duplicate when the winner lacks identity", () => {
    ...
    const staleIdentified = makeProject("stale-identified", "/work/gentic2", {
      repositoryIdentity: staleIdentity,
      updatedAt: "2026-07-01T00:00:00.000Z",
    });
    const freshIdentified = makeProject("fresh-identified", "/work/gentic2/", {
      updatedAt: "2026-07-02T00:00:00.000Z",
    });
    const winner = makeProject("winner", "/work/gentic2", {
      repositoryIdentity: null,
