...
import { classifyTaskAgentKind, type OrchestrationThreadActivity } from "@gentic2/contracts";
import {
...
describe("terminal robustness", () => {
  ...
  it("provider endedAt wins over ingestion time on the settling transition", () => {
    const agents = fold([
      activity("task.started", { taskId: "g2", taskType: "local_agent" }),
      activity(
        "task.updated",
        { taskId: "g2", status: "failed", endedAt: "2026-08-01T09:59:59.000Z" },
