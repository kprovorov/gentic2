...
describe("resolveMediaSource", () => {
  describe("direct URLs", () => {
    ...
    it.each(["data:image/png;base64,AAAA", "blob:https://app.gentic2.com/id"])(
      "loads %s directly",
      (href) => {
        expect(resolveMediaSource(href, { threadId, imageEmbed: true })).toMatchObject({
          access: "direct",
  ...
  it("serves G2 attachment files in place like any other host path", () => {
    const path = `/home/demo/.g2/userdata/attachments/${attachmentId}.mp4`;
