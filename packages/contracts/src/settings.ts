...
/**
 * The environment's theme, set with `g2 theme set <id>`. Each client applies
...
// Deliberately absent from ServerSettingsPatch: `g2 theme set` checks that an
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let Gentic2 spawn the server when needed.",
        providerSettingsForm: {
...
export const ServerSettings = Schema.Struct({
   ...
   * withholds the MCP credential, so the `gentic2` server (and with it every
  ...
  /**
   * Whether this server may install and run G2's device helper processes.
   ...
   * of the same value from one they already applied: `g2 theme set` must act
  ...
  /**
   * Null means inherit: the repository's g2.json, then "local". The old
   ...
   * chain also let g2.json outrank the environment. Null stays off the wire
  ...
  /**
   * Null defers to the repository's g2.json, then to recursive. A value
