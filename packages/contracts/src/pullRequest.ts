...
> = {
  ...
  forgejo: {
    ...
    unauthenticated:
      "Authenticate your Forgejo or Gitea server with `fj --host <server-url> auth add-token` on the Gentic2 server. If fj is missing or unconfigured for that server, use `tea login add`. A configured fj account must be repaired with fj.",
  },
  gitlab: {
    missing:
  ...
  bitbucket: {
    missing:
      "Bitbucket needs API credentials on the server. Set GENTIC2_BITBUCKET_EMAIL and GENTIC2_BITBUCKET_API_TOKEN, or GENTIC2_BITBUCKET_ACCESS_TOKEN.",
    unauthenticated:
      "Bitbucket rejected the configured credentials. Check GENTIC2_BITBUCKET_EMAIL and GENTIC2_BITBUCKET_API_TOKEN.",
  },
};
