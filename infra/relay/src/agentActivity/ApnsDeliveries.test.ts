  ...
  it.effect("queues Live Activity jobs with the device's APNs routing", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* deliveries.sendForTarget({
        target: {
          ...target,
          bundle_id: "com.gentic2.gentic2.preview",
  ...
  it.effect("sends signed jobs to the device's APNs environment and bundle topic", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(requests[0]?.headers["apns-topic"]).toBe(
        "com.gentic2.gentic2.preview.push-type.liveactivity",
      );
    }).pipe(
      Effect.provide(
        makeLayer({
          attempts,
          currentTargets: [
            { ...target, bundle_id: "com.gentic2.gentic2.preview", aps_environment: "sandbox" },
          ],
    ...
    for (const changed of ["bundle", "environment", "legacy"] as const) {
      it.effect(`routes ${kind} using current registration with ${changed} job metadata`, () => {
        ...
        const payload = makeApnsDeliveryJobPayload({
          ...
          ...(changed === "legacy"
            ? {}
            : { bundleId: "com.gentic2.gentic2.dev", apsEnvironment: "sandbox" as const }),
        ...
        return Effect.gen(function* () {
          ...
          expect(requests[0]?.headers["apns-topic"]).toBe(
            `${changed === "bundle" ? "com.gentic2.gentic2.preview" : "com.gentic2.gentic2.dev"}${kind === "live_activity_update" ? ".push-type.liveactivity" : ""}`,
          );
        }).pipe(
          Effect.provide(
            makeLayer({
              ...
              currentTargets: [
                {
                  ...
                  bundle_id:
                    changed === "bundle"
                      ? "com.gentic2.gentic2.preview"
                      : "com.gentic2.gentic2.dev",
