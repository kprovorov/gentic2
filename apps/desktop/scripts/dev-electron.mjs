...
const remoteDebuggingPort = process.env.GENTIC2_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line gentic2/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.GENTIC2_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.GENTIC2_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--gentic2-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--gentic2-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
