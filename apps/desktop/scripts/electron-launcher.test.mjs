...
describe("electron development launcher", () => {
  it("uses captured values only as fallbacks for a live runner environment", () => {
    ...
    assert.include(
      environmentScript,
      "if [ -z \"${GENTIC2_OTLP_PROTOCOL:-}\" ]; then export GENTIC2_OTLP_PROTOCOL='http/protobuf'; fi",
    );
  ...
  it("keeps the launcher script free of volatile environment values", () => {
    ...
    assert.include(
      script,
      "exec '/repo/node_modules/electron/Electron' --gentic2-dev-root='/repo/apps/desktop' '/repo/apps/desktop/dist-electron/main.cjs' \"$@\"",
    );
  });
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths(
      "/repo/apps/desktop/.electron-runtime/Gentic2 (Dev).app",
      "Gentic2 (Dev)",
    );

    assert.equal(paths.launcherExecutableName, "Gentic2 (Dev) Launcher");
    assert.equal(
      paths.launcherBinaryPath,
      "/repo/apps/desktop/.electron-runtime/Gentic2 (Dev).app/Contents/MacOS/Gentic2 (Dev) Launcher",
    );
    assert.equal(
      paths.runtimeElectronBinaryPath,
      "/repo/apps/desktop/.electron-runtime/Gentic2 (Dev).app/Contents/MacOS/Electron",
    );
    ...
    assert.include(
      script,
      "exec '/repo/apps/desktop/.electron-runtime/Gentic2 (Dev).app/Contents/MacOS/Electron'",
    );
    assert.notInclude(script, "node_modules/electron");
  });

  it("declares why the macOS app needs protected access", () => {
    const values = resolveMacBundleInfoPlistStrings("Gentic2 (Dev) Launcher");
    ...
    assert.equal(
      ...
      "Gentic2 reads project files you open in the desktop app.",
    );
  });

  it("ad-hoc signs the complete development app bundle", () => {
    assert.deepEqual(resolveMacCodeSignArguments("/runtime/Gentic2 (Dev).app"), [
      "--force",
      "--deep",
      "--sign",
      "-",
      "--timestamp=none",
      "/runtime/Gentic2 (Dev).app",
    ]);
  });

  it("restores execute permissions on an unchanged launcher", () => {
    const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "g2-launcher-"));
