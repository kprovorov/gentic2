...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "Gentic2");
  expect(findCaptureDestination([window(99, "Gentic2"), target], 42, "Gentic2")).toBe(target);
  expect(findCaptureDestination([window(99, "Gentic2")], 42, "Gentic2")).toBeUndefined();
...
it("does not guess between ambiguous windows from the same process", () => {
  expect(findCaptureDestination([window(42, "G2"), window(42, "G2")], 42, "G2")).toBeUndefined();
