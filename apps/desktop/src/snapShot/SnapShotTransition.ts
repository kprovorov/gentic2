...
export type SnapShotAnimationDestination = {
  ...
  /** Unit coordinates in G2's content area; GNOME supplies the real compositor origin. */
...
function createWindow(
  ...
  const window = new Electron.BrowserWindow({
    ...
    show: false,
    skipTaskbar: true,
    title: "Gentic2 Snapshot Animation",
    transparent: true,
    webPreferences: {
  ...
  private async showOverlays(
    ...
    try {
      ...
      if (this.waitForCompositorFrame) {
        // Cover the source with a composited snapshot before the caller reveals G2.
