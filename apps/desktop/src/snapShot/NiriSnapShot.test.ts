...
beforeEach(async () => {
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "g2-niri-test-"));
...
it("selects the native adapter without needing a portal or GNOME extension", async () => {
  expect(await getLinuxCaptureSupport("com.gentic2.Gentic2")).toEqual({
    linuxBackend: "niri",
    linuxFeedbackAvailable: false,
  });
  const snapshot = await captureLinuxWindow("com.gentic2.Gentic2");
...
it("does not activate G2 until requested, then matches PID and title", async () => {
  ...
  windows = [
    { ...window, id: 1, pid: 999, title: "Gentic2" },
    { ...window, id: 2, pid: process.pid, title: "Other G2" },
    { ...window, id: 3, pid: process.pid, title: "Gentic2" },
  ];
  await snapshot.feedback!.activate("Gentic2");
  expect(calls).toContainEqual({ Action: { FocusWindow: { id: 3 } } });
});

it("waits for the restored G2 window to map instead of polling", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  ...
  handler = async (request, socket) => {
    ...
    if (request === "EventStream")
      send(socket, {
        WindowOpenedOrChanged: { window: { ...window, id: 4, pid: process.pid, title: "Gentic2" } },
      });
  };
  await snapshot.feedback!.activate("Gentic2");
...
it("rejects ambiguous activation targets", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  windows = [1, 2].map((id) => ({ ...window, id, pid: process.pid, title: "Gentic2" }));
  await expect(snapshot.feedback!.activate("Gentic2")).rejects.toThrow("More than one");
});
...
it("cancels pending activation when capture feedback is closed", async () => {
  ...
  const activation = expect(snapshot.feedback!.activate("Gentic2")).rejects.toThrow("cancelled");
...
it("rejects compositor errors and cleans up its temporary image", async () => {
  ...
  await expect(captureLinuxWindow("com.gentic2.Gentic2")).rejects.toThrow("window disappeared");
  expect(await NodeFSP.stat(NodePath.dirname(capturePath!)).catch(() => undefined)).toBeUndefined();
});
