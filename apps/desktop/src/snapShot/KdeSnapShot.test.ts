...
beforeEach(async () => {
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "g2-kde-test-"));
  paths = {
...
it("captures with verified identity, cleans private files, and activates only the owning G2 PID/title", async () => {
  await setup.perform("install-kde-helper");
