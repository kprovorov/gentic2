...
export const HYPRLAND_CAPTURE_EXECUTABLE = "g2-hyprland-snap-shot";
...
export function hyprlandCaptureExecutable(paths: HyprlandCapturePaths) {
  return NodePath.join(paths.dataHome, "gentic2", "hyprland-capture", HYPRLAND_CAPTURE_EXECUTABLE);
}
...
export class HyprlandCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        throw new Error(
          "The Hyprland capture helper is missing from this build. Update or reinstall Gentic2.",
        );
...
export async function captureHyprlandWindow(
  ...
  const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "g2-hyprland-capture-"));
