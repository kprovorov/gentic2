...
it.effect.each(["win32", "darwin", "linux"] as const)(
  ...
  (platform) => {
    ...
    const g2 = {
      id: 42,
      title: "Gentic2",
      appIdentifier: "com.gentic2.Gentic2.desktop",
      owner: { name: "Gentic2", processId: 123 },
      bounds,
    ...
    macCaptureMock.mockReset().mockImplementation(async () => {
      images.push(g2.png);
      return { source: { name: g2.title }, png: g2.png };
    });
    ...
    linuxCaptureMock.mockResolvedValueOnce({
      ...
      window: {
        title: g2.title,
        appName: g2.owner.name,
        appIdentifier: g2.appIdentifier,
        processId: g2.owner.processId,
        bounds,
      },
      feedback: {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        const saved = yield* decodePendingMetadata(metadata);
        assert.equal(saved.source.windowTitle, g2.title);
        assert.equal(saved.source.appName, g2.owner.name);
        assert.equal(saved.source.accessibleText, `Window from process ${g2.owner.processId}`);
        assert.deepEqual(images, [g2.png]);
        assert.equal(prepareCaptureRevealMock.mock.calls.length, platform === "win32" ? 1 : 0);
        if (platform === "linux") {
          assert.equal(saved.source.appIdentifier, g2.appIdentifier);
          assert.deepEqual(activate.mock.calls, [[g2.title]]);
        }
      }),
    ).pipe(
      Effect.provide(
      ...
      Effect.gen(function* () {
        ...
        const warning = logs.find(
          (message) =>
            ...
            message[0] === "The compositor could not activate Gentic2 after the snapshot",
        );
...
it.each([
  { names: ["⠙ gentic2"], expected: "Verified text" },
  { names: ["⠋ gentic2", "⠙ gentic2"], expected: undefined },
...
it.effect("flags revoked macOS permissions on read and re-registers once they return", () => {
  ...
  return Effect.scoped(
    Effect.gen(function* () {
      ...
      assert.equal(
        revoked.message,
        "Allow Screen Recording in System Settings, then restart Gentic2.",
      );
...
for (const fails of [false, true]) {
  it.effect(`tests macOS capture without publishing it and cleans up, failure=${fails}`, () => {
    const active = {
      ...
      id: 42,
      title: "Setup",
      owner: { name: "Gentic2", processId: 123, path: "/Applications/Gentic2.app" },
      bounds: { x: 0, y: 0, width: 800, height: 600 },
    };
    activeWindowMock.mockReset().mockResolvedValue(active);
