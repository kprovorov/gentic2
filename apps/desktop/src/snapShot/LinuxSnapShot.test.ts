...
const appId = "com.gentic2.Gentic2";
const metadata = {
  title: "Editor",
  appName: "Text Editor",
...
beforeEach(async () => {
  ...
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "g2-portal-test-"));
...
it("retains the authenticated connection until the compositor flight has landed", async () => {
  ...
  await feedback.activate("Gentic2");
  ...
  expect(
    bus.calls
      ...
      .map((call) => call.body),
  ).toEqual([[false, true], ["Gentic2"], [0.1, 0.8, 0.2, 0.1]]);
  expect(bus.disconnect).toHaveBeenCalledOnce();
  feedback.close();
  expect(bus.disconnect).toHaveBeenCalledOnce();
});
...
it("can activate without animation, and expires a renderer-abandoned capture", async () => {
  ...
  await expect(snapshot!.feedback!.activate("Gentic2")).rejects.toThrow("No window");
