...
import * as SshAuth from "@gentic2/ssh/auth";
import { resolveSshTarget } from "@gentic2/ssh/command";
import { discoverSshHosts } from "@gentic2/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@gentic2/ssh/errors";
import * as SshTunnel from "@gentic2/ssh/tunnel";
...
>()("@gentic2/desktop/ssh/DesktopSshEnvironment") {}

export interface DesktopSshEnvironmentLayerOptions {
  readonly resolveCliRunner?: Effect.Effect<SshTunnel.RemoteG2RunnerOptions>;
}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Gentic2 window is not available for SSH authentication.";
      break;
