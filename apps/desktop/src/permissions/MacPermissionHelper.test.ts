...
vi.mock("electron", async () => {
  ...
  return {
    app: {
      getPath: () => "/Applications/Gentic2 (Nightly).app/Contents/MacOS/Gentic2",
    },
...
describe("macAppBundlePath", () => {
  it("resolves bundles with spaces and refuses non-bundle executables", () => {
    expect(macAppBundlePath("/Applications/Gentic2.app/Contents/MacOS/Gentic2")).toBe(
      "/Applications/Gentic2.app",
    );
    expect(macAppBundlePath("/usr/local/bin/electron")).toBeUndefined();
    expect(macAppBundlePath("/Applications/Gentic2.app/other/MacOS/Gentic2")).toBeUndefined();
  });
});
it("drags the running app bundle only for the helper's own renderer", async () => {
  ...
  expect(mocks.startDrag).toHaveBeenCalledWith({
    file: "/Applications/Gentic2 (Nightly).app",
    icon: mocks.createFromPath.mock.results[0]!.value.resize(),
  });
  send("finder");
  expect(mocks.showItemInFolder).toHaveBeenCalledWith("/Applications/Gentic2 (Nightly).app");
});
...
it("does not show a helper with a missing packaged icon", async () => {
  ...
  await expect(open()).rejects.toThrow("packaged Gentic2 icon is missing");
  expect(windows).toHaveLength(0);
});
