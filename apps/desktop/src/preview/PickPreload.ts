...
const OVERLAY_ATTRIBUTE = "data-gentic2-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--g2-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--g2-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--g2-radius": theme.radius,
    "--g2-background": theme.background,
    "--g2-foreground": theme.foreground,
    "--g2-popover": theme.popover,
    "--g2-popover-foreground": theme.popoverForeground,
    "--g2-primary": theme.primary,
    "--g2-primary-foreground": theme.primaryForeground,
    "--g2-muted": theme.muted,
    "--g2-muted-foreground": theme.mutedForeground,
    "--g2-accent": theme.accent,
    "--g2-accent-foreground": theme.accentForeground,
    "--g2-border": theme.border,
    "--g2-input": theme.input,
    "--g2-ring": theme.ring,
    "--g2-font-sans": theme.fontSans,
    "--g2-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-gentic2-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--g2-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-gentic2-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
