...
const makeNativeOperations = Effect.fn("PreviewManager.makeOperations")(function* (
  ...
  const ensurePlaywrightInjected = Effect.fn("PreviewManager.ensurePlaywrightInjected")(function* (
    ...
    const installed = yield* evaluateWithDebugger<boolean>(
      tabId,
      send,
      "Boolean(globalThis.__g2PlaywrightInjected)",
      true,
    );
    if (installed) return;
  ...
  const resolveClickPoint = Effect.fn("PreviewManager.resolveClickPoint")(function* (
    ...
    >(
      ...
      `(() => {
          try {
            const injected = globalThis.__g2PlaywrightInjected;
  ...
  const typeIntoAutomationTarget = Effect.fn("PreviewManager.typeIntoAutomationTarget")(function* (
    ...
    >(
      ...
      `(() => {
          try {
            const element = ${locatorJson ? `(() => { const injected = globalThis.__g2PlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, true); })()` : "document.activeElement"};
            if (!element) return { notFound: true };
            const textControl =
  ...
  const withNativeKeyReceipt = Effect.fn("PreviewManager.withNativeKeyReceipt")(function* (
    ...
    const { frames, receiptKey } = yield* Effect.acquireRelease(
      attempt(context, () => ({
        ...
        receiptKey: JSON.stringify(`__g2NativeKey_${NodeCrypto.randomUUID()}`),
      })),
      ({ frames, receiptKey }) =>
        Effect.forEach(
          frames,
    ...
    while (true) {
      ...
      if (
        ...
        const world = (yield* send(
          ...
          {
            frameId,
            worldName: "g2-preview-key-target",
          },
          sessionId,
    ...
    yield* Effect.gen(function* () {
      ...
      if (keySequence.commands?.length) {
        ...
        const selectionKey = yield* encodeJson(
          context,
          `__g2EditingSelection_${NodeCrypto.randomUUID()}`,
        );
  ...
  const performAutomationScroll = Effect.fn("PreviewManager.performAutomationScroll")(function* (
    ...
    >(
      ...
      `(() => {
        try {
          const target = ${locatorJson ? `(() => { const injected = globalThis.__g2PlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, true); })()` : "window"};
  ...
  const performAutomationWaitFor = Effect.fn("PreviewManager.performAutomationWaitFor")(function* (
    ...
    while ((yield* currentMillis) <= deadline) {
      ...
      >(
        ...
        `(() => {
              try {
                const selectorMatched = ${locatorJson ? `(() => { const injected = globalThis.__g2PlaywrightInjected; return injected.querySelector(injected.parseSelector(${locatorJson}), document, false) !== null; })()` : "true"};
                const textMatched = ${
...
>()("@gentic2/desktop/preview/Manager/PreviewManager") {}
