...
describe("ElectronProtocol", () => {
  ...
  it.effect("serves the bundled client from disk without a backend", () =>
    Effect.gen(function* () {
      ...
      const request = (pathname: string, init?: RequestInit) =>
        Effect.promise(() => handler!(new Request(`gentic2://app${pathname}`, init)));
  ...
  it.effect("proxies the stable renderer origin to the current app server", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          yield* protocol.registerDesktopProtocol({
            ...
            clerkFrontendApiHostname: "clerk.gentic2.com",
          });
          assert.isDefined(handler);

          const response = yield* Effect.promise(() =>
            handler!(
              new Request("gentic2-dev://app/api/health?verbose=1", {
                headers: {
                  accept: "application/json",
                  origin: "gentic2-dev://app",
                  referer: "gentic2-dev://app/",
          ...
          assert.include(
            ...
            "script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval' https://clerk.gentic2.com https://challenges.cloudflare.com",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
          ...
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "img-src 'self' gentic2-dev: blob: data: http: https:",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "font-src 'self' gentic2-dev: data:",
          );
        }),
      );
      ...
      assert.deepEqual(unhandleMock.mock.calls, [["gentic2-dev"]]);
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("gentic2://other/")));
        }),
      );
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("gentic2-dev://app/")));
        }),
      );
  ...
  it.effect("preserves protocol registration failures", () =>
    Effect.gen(function* () {
      ...
      assert.equal(error.scheme, "gentic2-dev");
      assert.strictEqual(error.cause, cause);
      assert.equal(error.message, 'Failed to register Electron protocol scheme "gentic2-dev".');
    }).pipe(Effect.provide(protocolLayer)),
  );
  ...
  it.effect("preserves protocol unregistration failures", () =>
    Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(error.scheme, "gentic2");
        assert.strictEqual(error.cause, cause);
        assert.equal(error.message, 'Failed to unregister Electron protocol scheme "gentic2".');
      }
    }).pipe(Effect.provide(protocolLayer)),
  );
  ...
  it("keeps executable sources host-restricted while allowing runtime network resources", () => {
    ...
    assert.deepEqual(directives["media-src"], ["'self'", "gentic2:", "blob:", "http:", "https:"]);
    assert.deepEqual(directives["frame-src"], ["'self'", "blob:", "http:", "https:"]);
    assert.deepEqual(directives["font-src"], ["'self'", "gentic2:", "data:"]);
  });
});
