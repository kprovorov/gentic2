...
describe("ElectronWindow", () => {
    ...
    Effect.gen(function* () {
      ...
      const options = {
        ...
        webPreferences: {
          ...
          partition: "persist:gentic2-preview-test",
          sandbox: true,
      ...
      assert.equal(error.message, 'Failed to create Electron BrowserWindow "Gentic2" (1100x780).');
  ...
  it.effect("focuses the exact G2 window before activating from a shell-hosted app", () =>
    Effect.gen(function* () {
      const operations: Array<string> = [];
  ...
  it.effect("prepares the exact G2 window before a capture overlay", () =>
    Effect.gen(function* () {
