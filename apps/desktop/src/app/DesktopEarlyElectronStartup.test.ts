...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("reads the persisted linux password-store preference before Electron is ready", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: { GENTIC2_HOME: "/home/user/.g2-test" },
      homeDirectory: "/home/user",
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.g2-test/userdata/desktop-settings.json");
  ...
  it("resolves the early linux Electron switches", () => {
    const options = resolveEarlyLinuxElectronOptions({
      env: {
        GENTIC2_HOME: "/home/user/.g2-test",
    ...
    assert.deepEqual(options, {
      isDevelopment: true,
      linuxWmClass: "gentic2-dev",
      linuxDesktopEntryName: "com.gentic2.Gentic2.Development.desktop",
      passwordStore: "gnome-libsecret",
    });
  });

  it("keeps implicit development state under ~/.g2/dev when GENTIC2_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.g2/dev/desktop-settings.json");
  ...
  it("treats whitespace-only GENTIC2_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        GENTIC2_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
