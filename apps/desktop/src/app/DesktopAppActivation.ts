...
import { HostProcessUserId } from "@gentic2/shared/hostProcess";
...
/**
 * Serves `g2 app` requests on the local control address until `close`.
 *
  ...
  const handleConnection = (socket: NodeNet.Socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      void input.handle(parsed).then(finish, () => {
        finish(
          invalidResponse(parsed.requestId, "Gentic2 could not process the desktop app request."),
        );
      });
    });
...
>()("@gentic2/desktop/app/DesktopAppActivation") {}
