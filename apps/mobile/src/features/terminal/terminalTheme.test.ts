...
import { BUILT_IN_THEMES, getThemeColorsForAppearance } from "@gentic2/shared/themePalettes";
...
describe("getMobileTerminalTheme", () => {
  it("uses the shared default light terminal colors", () => {
    expect(getMobileTerminalTheme("gentic2", "light")).toMatchObject({
      background: "#fcfcfc",
      foreground: "#27272a",
  ...
  it("uses the shared default dark terminal colors", () => {
    expect(getMobileTerminalTheme("gentic2", "dark")).toMatchObject({
      background: "#0a0a0a",
      foreground: "#f5f5f5",
  ...
  it("applies the selected palette without replacing ANSI status colors", () => {
    const standard = getMobileTerminalTheme("gentic2", "dark");
...
describe("buildGhosttyThemeConfig", () => {
  it("serializes theme colors into a ghostty config file", () => {
    const config = buildGhosttyThemeConfig(getMobileTerminalTheme("gentic2", "dark"));
