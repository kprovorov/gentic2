...
describe("fileRoutePathSegments", () => {
  it("round-trips workspace-relative and host paths through the route", () => {
    ...
    expect(fileRoutePathSegments("/tmp/g2-cleanup/report.md").join("/")).toBe(
      "/tmp/g2-cleanup/report.md",
    );
  });
});
...
describe("fileHeaderSubtitle", () => {
  it("places a workspace file under its project", () => {
    expect(
      fileHeaderSubtitle("gentic2", "apps/mobile/src/features/threads/fileChipMenu.test.ts"),
    ).toBe("gentic2 · apps/mobile/src/features/threads");
  });
  ...
  it("shows only the directory for a host file outside the workspace", () => {
    ...
    expect(fileHeaderSubtitle("gentic2", "/tmp/report.md")).toBe("/tmp");
  });

  it("shows only the project for a file at the workspace root", () => {
    expect(fileHeaderSubtitle("gentic2", "README.md")).toBe("gentic2");
  });
});
