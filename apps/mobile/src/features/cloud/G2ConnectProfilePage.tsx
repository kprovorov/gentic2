...
function confirmDeregister(environment: RelayClientEnvironmentRecord, onConfirm: () => void) {
  ...
  const message = `“${environment.label}” will be removed from this account. Gentic2 Connect access will be revoked, any managed tunnel will be removed, and a host space will become available. Local connections on your devices are not changed.`;
...
/**
 * The "Gentic2 Connect" custom page inside Clerk's native user profile: every
 ...
 */
export function G2ConnectProfilePage() {
  ...
  const handleDeregister = async (environment: RelayClientEnvironmentRecord) => {
    ...
    console.error("[g2-connect] Could not deregister environment", {
      environmentId: environment.environmentId,
      message,
      traceId,
      cause,
  ...
  return (
    ...
    >
      ...
      {environmentsState.error ? (
        <>
          <ClerkRow
            title="Could not load Gentic2 Connect environments"
        ...
        <ClerkRow
          ...
          subtitle="Link a server from its local Settings to reach it through Gentic2 Connect."
        />
      )}
