...
describe("resolveNativeReviewDiffView", () => {
  ...
  it("returns the payload bridge when the native review diff view is installed", async () => {
    ...
    expect(expoMocks.requireNativeView).toHaveBeenCalledWith("G2ReviewDiffSurface");
  });
  ...
  it("returns null when the view manager cannot be required", async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        nativeModuleName: "G2ReviewDiffSurface",
        cause,
      }),
    );
  ...
  it("recognizes registration races for the installed native view name", async () => {
    ...
    expect(
      isPendingNativeViewRegistration(
        new Error("Unable to find the 'G2ReviewDiffSurface' view for this native tag"),
      ),
    ).toBe(true);
    expect(
      isPendingNativeViewRegistration(
        new Error("Unable to find the 'G2ReviewDiffView' view for this native tag"),
      ),
    ).toBe(false);
    expect(
      isPendingNativeViewRegistration(
        new Error(
          "Unable to find the class expo.modules.g2reviewdiff.G2ReviewDiffView view with tag 1150",
        ),
      ),
    ).toBe(true);
  });
