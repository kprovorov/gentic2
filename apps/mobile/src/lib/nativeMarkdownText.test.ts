...
} from "@gentic2/mobile-markdown-text/markdown";
...
describe("nativeMarkdownTextRuns", () => {
  ...
  it("maps rendered selection offsets back to canonical references without losing repeated chips", () => {
    const href = "g2-context://v1/image/screenshot";
    expect(
      nativeMarkdownContextCopyRanges([
      ...
      { start: 3, end: 12, text: "![Checkout](g2-context://v1/image/screenshot)" },
      { start: 18, end: 28, text: "![Checkout](g2-context://v1/image/screenshot)" },
    ]);
  });
  it("restores canonical skill and context text from Android's single-image chips", () => {
    expect(
      nativeMarkdownContextCopyRanges([
        ...
        {
          run: { text: "Screenshot", href: "g2-context://v1/image/screenshot" },
          text: "",
          inlineImageLength: 1,
        },
      ]),
      ...
      { start: 9, end: 10, text: "![Screenshot](g2-context://v1/image/screenshot)" },
    ]);
  });
...
describe("nativeMarkdownDocumentRuns", () => {
  it("renders a file mention without swallowing sentence punctuation or changing package references", () => {
      ...
      children: [
        {
          type: "paragraph",
          children: [
            { type: "text", content: "Inspect @src/Checkout.tsx. Use @gentic2/contracts." },
          ],
        },
      ],
    });
    expect(runs).toEqual([
      ...
      { text: ". Use @gentic2/contracts.", role: "body" },
    ]);
  });
  ...
  it("keeps adjacent context links with the same href in separate runs", () => {
    const href = "g2-context://v1/terminal/ctx-1";
...
describe("composerChipSizeSuffix", () => {
  it("labels attachment records with a human size, matching web's chip", async () => {
    const { composerChipSizeSuffix } = await import("@gentic2/mobile-markdown-text/markdown");
...
describe("contextChipPresentation image detection", () => {
  it("treats a picture attached through the file picker as an image", async () => {
    const { contextChipPresentation } = await import("@gentic2/mobile-markdown-text/markdown");
