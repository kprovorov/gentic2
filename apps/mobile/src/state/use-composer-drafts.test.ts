...
function contextDraft(start: number, count: number): ComposerDraft {
  ...
  return {
    text: records.map((record) => `[Skill](g2-context://v1/skill/${record.contextId})`).join(" "),
...
describe("mobile composer drafts", () => {
  it.each([false, true])(
    ...
    async (archived) => {
      ...
      expect(restored?.text).toBe("Review these [notes.txt](g2-context://v1/file/legacy-file) ");
  ...
  it("restores a missing file reference without duplicating its record or replacing another record's id", () => {
    ...
    for (const records of [[existing, skill], [skill]]) {
      ...
      expect(restored?.text).toBe(
        `[notes.txt](g2-context://v1/file/${records.length === 2 ? "original" : "file_2"}) `,
      );
    }
  });
  it.each([false, true])(
    ...
    async (uploaded) => {
      ...
      const file = {
        ...
        mimeType: "text/plain",
        sizeBytes: 1,
        fileUri: "file:///documents/g2-composer-attachments/undo-file.txt",
        ...(uploaded
          ? {
  ...
  it("prunes unreferenced context during a content merge", () => {
    ...
    const merged = mergeComposerDraftContentState(
      { key: { ...existing, text: "[Skill](g2-context://v1/skill/skill-0)" } },
      "key",
      { ...incoming, text: "[Skill](g2-context://v1/skill/skill-2)" },
    );
  ...
  it("rejects attachments atomically when no context slots remain", async () => {
    ...
    appAtomRegistry.set(composerDraftsAtom, {
      [key]: {
        text: records
          .map((record) => `[Skill](g2-context://v1/skill/${record.contextId})`)
          .join(" "),
        context: { version: 1, records },
        attachments: [],
      },
  ...
  it.each(["environment-1:thread", "environment-1:new-task:draft"])(
    ...
    async (key) => {
      ...
      const draft = getComposerDraftSnapshot(key);
      expect(draft.text).toBe("before [pasted-text.txt](g2-context://v1/file/paste) after");
  ...
  it.each(["attachment", "context", "imported context"])(
    ...
    async (kind) => {
      ...
      const firstLink = "[notes-0.txt](g2-context://v1/file/file-0)";
  ...
  it("rejects an imported file atomically when edits during import use up its replacement slot", async () => {
    ...
    appendComposerDraftAttachments(key, files, { appendReference: true });
    const firstLink = "[notes-0.txt](g2-context://v1/file/existing-0)";
    const insertion = captureComposerDraftInsertion(key, { start: 0, end: firstLink.length });
  ...
  it("inserts context at the saved caret and retains its payload through persistence and restore", () => {
    ...
    const reference = "[Build output](g2-context://v1/terminal/context-terminal)";
  ...
  it("hydrates generic file attachments from their saved local paths", () => {
    ...
    expect(
      ...
      "environment-1:thread-1": {
        text: "Review this file [report.pdf](g2-context://v1/file/file-1) ",
        attachments: [file],
        context: {
          version: 1,
          records: [
  ...
  it("releases videos rejected by the live draft limit and keeps accepted files", async () => {
    ...
    const makeAttachment = (id: string) => ({
      ...
      sizeBytes: 42,
      fileUri: `file:///documents/g2-composer-attachments/${id}.mov`,
    });
  ...
  it("keeps shared attachment files until every draft releases them", async () => {
    ...
    const file = {
      ...
      mimeType: "application/pdf",
      sizeBytes: 42,
      fileUri: "file:///documents/g2-composer-attachments/report.pdf",
    };
    appAtomRegistry.set(composerDraftsAtom, {
  ...
  it("retains a referenced file-backed image and releases it once unreferenced", async () => {
    ...
    const image = {
      ...
      mimeType: "image/png",
      sizeBytes: 3,
      fileUri: "file:///documents/g2-composer-attachments/photo.png",
      previewUri: "file:///documents/g2-composer-attachments/photo.png",
    };
    appAtomRegistry.set(composerDraftsAtom, {
  ...
  it("keeps a failed-send draft's pending upload for retry", async () => {
    ...
    const file = {
      ...
      mimeType: "application/pdf",
      sizeBytes: 42,
      fileUri: "file:///documents/g2-composer-attachments/failed-send.pdf",
  ...
  it.each(["file", "image"] as const)(
    ...
    async (type) => {
      ...
      const metadata = {
        ...
        sizeBytes: 42,
        fileUri: `file:///documents/g2-composer-attachments/${name}`,
        uploadEnvironmentId: environmentId,
      ...
      expect(getComposerDraftSnapshot(key)).toEqual(
        type === "image"
          ...
          : {
              text: "Unsent notes [notes.pdf](g2-context://v1/file/local-notes) ",
              attachments: [file],
              context: {
                version: 1,
                records: [
      ...
      expect(getComposerDraftSnapshot("pending-task:queued-1").text).toBe(
        type === "image"
          ? "Edited queued task"
          : "Edited queued task [notes.pdf](g2-context://v1/file/local-notes) ",
      );
  ...
  it.each(["file", "image"] as const)(
    ...
    async (type) => {
      ...
      const metadata = {
        ...
        sizeBytes: 42,
        fileUri: `file:///private/var/mobile/Containers/Data/Application/11111111-1111-4111-8111-111111111111/Documents/g2-composer-attachments/${fileName}`,
      };
      const file =
        type === "image"
          ? { ...metadata, type, previewUri: metadata.fileUri }
      ...
      const currentFile = {
        ...file,
        fileUri: `file:///var/mobile/Containers/Data/Application/22222222-2222-4222-8222-222222222222/Documents/g2-composer-attachments/${fileName}`,
      };
  ...
  it("preserves a preview opened while cleanup is checking the incoming inbox", async () => {
    ...
    const file = {
      ...
      mimeType: "video/mp4",
      sizeBytes: 42,
      fileUri: "file:///documents/g2-composer-attachments/recording.mp4",
    };
  ...
  it("removes an unreferenced local file and its pending upload", async () => {
    ...
    const file = {
      ...
      mimeType: "application/pdf",
      sizeBytes: 42,
      fileUri: "file:///documents/g2-composer-attachments/discarded.pdf",
  ...
  it("keeps a pending upload referenced through another local file", async () => {
    ...
    const discarded = {
      ...
      mimeType: "application/pdf",
      sizeBytes: 42,
      fileUri: "file:///documents/g2-composer-attachments/discarded-copy.pdf",
    ...
    const retained = {
      ...
      fileUri: "file:///documents/g2-composer-attachments/retained-copy.pdf",
    };
    appAtomRegistry.set(composerDraftsAtom, {
  ...
  it("completes local cleanup when pending upload deletion fails", async () => {
    ...
    const file = {
      ...
      mimeType: "application/pdf",
      sizeBytes: 42,
      fileUri: "file:///documents/g2-composer-attachments/delete-failed.pdf",
  ...
  it("keeps a file until its incoming share is consumed", async () => {
    ...
    const file = {
      ...
      mimeType: "application/pdf",
      sizeBytes: 42,
      fileUri: "file:///documents/g2-composer-attachments/incoming.pdf",
    };
  ...
  it("does not delete files when incoming share ownership cannot be loaded", async () => {
    ...
    const file = {
      ...
      mimeType: "application/pdf",
      sizeBytes: 42,
      fileUri: "file:///documents/g2-composer-attachments/incoming-unknown.pdf",
    };
    const warning = vi.spyOn(console, "warn").mockImplementation(() => undefined);
  ...
  it("takes out only what the merge inserted when the user edited during it", () => {
    ...
    const keptAttachment = {
      ...
      mimeType: "application/pdf",
      sizeBytes: 1,
      fileUri: "file:///documents/g2-composer-attachments/kept.pdf",
    };
    const insertedAttachment = {
      ...
      mimeType: "application/pdf",
      sizeBytes: 1,
      fileUri: "file:///documents/g2-composer-attachments/inserted.pdf",
    };
  ...
  it("spares a file re-owned between the sweep's scan and its deletion", async () => {
    ...
    const fileFor = (id: string) => ({
      ...
      mimeType: "application/pdf",
      sizeBytes: 42,
      fileUri: `file:///documents/g2-composer-attachments/${id}.pdf`,
    });
  ...
  it("keeps attachment files and uploads after a recovered message leaves an incomplete outbox", async () => {
    ...
    const file = {
      ...
      mimeType: "application/pdf",
      sizeBytes: 42,
      fileUri: "file:///documents/g2-composer-attachments/shared.pdf",
      uploadEnvironmentId: EnvironmentId.make("environment-1"),
  ...
  it("hydrates persisted drafts before a cold-start sweep deletes their files", async () => {
    ...
    expect(freshRegistry.get(fresh.composerDraftsAtom)).toEqual({
      "environment-1:thread-1": {
        text: "Persisted draft [report.pdf](g2-context://v1/file/file-cold-start) ",
        attachments: [file],
        context: {
          version: 1,
          records: [
