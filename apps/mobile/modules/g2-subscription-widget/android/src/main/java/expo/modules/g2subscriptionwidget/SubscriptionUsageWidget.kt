...
class SubscriptionUsageWidget : AppWidgetProvider() {
  ...
  companion object {
    const val PREFERENCES = "g2_subscription_widget"
    private const val EXPIRE = "expo.modules.g2subscriptionwidget.EXPIRE"
    ...
    private fun update(context: Context, manager: AppWidgetManager, id: Int) {
      ...
      val views = RemoteViews(context.packageName, R.layout.g2_subscription_widget)
      ...
      if (limits > 0) {
        views.setTextViewText(
          R.id.g2_widget_title,
          context.getString(R.string.g2_subscription_widget_title_count, limits)
        )
        views.setContentDescription(
          R.id.g2_widget_title,
          context.resources.getQuantityString(
            R.plurals.g2_subscription_widget_title_description,
            limits,
            limits
          )
        )
      }
      openApp?.let { views.setOnClickPendingIntent(R.id.g2_widget_root, it) }
      openAppIntent(context, id, snapshot, forCollection = true)?.let {
        views.setPendingIntentTemplate(R.id.g2_widget_rows, it)
      }
      ...
      views.setRemoteAdapter(R.id.g2_widget_rows, items.build())
      views.setEmptyView(R.id.g2_widget_rows, R.id.g2_widget_empty)
      ...
      val checked = if (checkedAt > 0) {
        ...
        context.getString(R.string.g2_subscription_widget_last_checked, formatted)
      } else {
        context.getString(R.string.g2_subscription_widget_unknown_check)
      }
      views.setTextViewText(R.id.g2_widget_footer, checked)
    ...
    private fun openAppIntent(
      ...
      val deepLink = snapshot?.optString("url")?.takeIf { it.isNotBlank() }
        ?: "gentic2://settings/usage?tab=limits"
    ...
    private fun rowView(context: Context, provider: JSONObject, window: JSONObject?): RemoteViews {
      val child = RemoteViews(context.packageName, R.layout.g2_subscription_widget_row)
      ...
      child.setTextViewText(R.id.g2_widget_label, label)
      child.setTextViewText(R.id.g2_widget_window, windowLabel)
      val percent = remaining?.let {
        context.getString(R.string.g2_subscription_widget_remaining, it)
      } ?: "—"
      child.setTextViewText(R.id.g2_widget_percent, percent)
      val visibility = if (remaining == null) View.GONE else View.VISIBLE
      child.setViewVisibility(R.id.g2_widget_progress, visibility)
      if (remaining != null) child.setProgressBar(R.id.g2_widget_progress, 100, remaining, false)
      val reset = window?.optString("reset")
        ?: context.getString(R.string.g2_subscription_widget_refresh)
      child.setTextViewText(R.id.g2_widget_reset, reset)
      child.setOnClickFillInIntent(R.id.g2_widget_row, Intent())
