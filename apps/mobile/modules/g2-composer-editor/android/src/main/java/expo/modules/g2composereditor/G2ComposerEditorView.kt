...
import expo.modules.g2markdowntext.G2ContextChip
import org.json.JSONObject
import kotlin.math.max

class G2ComposerEditorView(context: Context, appContext: AppContext) : ExpoView(
  context,
  appContext
) {
  ...
  private fun applyTokenSpans() {
    ...
    tokens.forEach { token ->
      ...
      editable.setSpan(
        ComposerChipSpan(
          G2ContextChip(
            content = G2ContextChip.Content(
              label = token.label,
              symbol = token.symbol,
              detail = token.detail
            ),
            fontSize = editor.textSize * 0.8f,
            colors = G2ContextChip.Colors(
              accent = G2ContextChip.color(token.accent, chipTheme.chipText),
...
private class ComposerChipSpan(
  private val chip: G2ContextChip
) : ReplacementSpan() {
...
internal class SelectionAwareEditText(context: Context) : EditText(context) {
  ...
  private fun copyContext(cut: Boolean): Boolean {
    ...
    G2ComposerClipboard.write(context, text.substring(start, end), clipboardFragment)
    if (cut) text.delete(start, end)
    return true
  }

  private fun pasteContextOrImages(): Boolean {
    val payload = G2ComposerClipboard.read(context)
    if (payload["html"]?.contains("data-g2-context-fragment=") == true) {
