# G2 Mobile Terminal Native Module
...
custom-I/O and live-padding branch. `G2TerminalView` owns a `libghostty` surface and uses that
callback I/O model:
...
`gentic2/custom-io-ordered-feed` branch when vendored, based on VVTerm's
`vvterm/custom-io-padding` branch).

```bash
apps/mobile/modules/g2-terminal/scripts/build-libghostty-ios16.sh
```
...
```bash
apps/mobile/modules/g2-terminal/scripts/build-libghostty-android.sh
```
