...
final class G2NativeFilePresentation: NSObject, QLPreviewControllerDataSource,
  QLPreviewControllerDelegate, UIAdaptivePresentationControllerDelegate {
  let identifier: String
  private var controller: UIViewController?
  private let completion: (Error?) -> Void
  private weak var sources: G2PresentationSources?
  ...
  init(identifier: String, sources: G2PresentationSources, sourceIdentifier: String, completion: @escaping (Error?) -> Void) {
  ...
  nonisolated private static func prepareFile(url: URL, title: String) async throws -> URL {
    ...
    let directory = FileManager.default.temporaryDirectory.appendingPathComponent("g2-preview-\(UUID().uuidString)")
