...
public final class G2KeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("G2KeyboardCommands")

    View(G2KeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: G2KeyboardCommandsView, commands: [String]) in
...
public final class G2KeyboardCommandsView: ExpoView {
  ...
  public override func canPerformAction(_ action: Selector, withSender sender: Any?) -> Bool {
    if action == #selector(openCommandPalette) || action == #selector(paletteNext) || action == #selector(palettePrevious) || action == #selector(paletteDismiss),
       let input = window?.g2FirstResponder as? UITextInput,
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.g2FirstResponder == nil else { return }
...
private extension UIView {
  var g2FirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.g2FirstResponder { return responder }
    }
    return nil
  }
}
