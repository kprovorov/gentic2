...
public final class G2NativeControlsModule: Module {
  private let presentationSources = G2PresentationSources()
  private var videoPresentation: G2NativeVideoPresentation?
  private var filePresentation: G2NativeFilePresentation?

  public func definition() -> ModuleDefinition {
    Name("G2NativeControls")
    ...
    View(G2PresentationSourceView.self) {
      ViewName("PresentationSource")
      Prop("identifier") { (view: G2PresentationSourceView, identifier: String) in
        view.sources = self.presentationSources
        view.identifier = identifier
      }
    }

    View(G2ContextSheetSizeView.self) {
      ViewName("ContextSheetSize")
      Prop("contentHeight") { (view: G2ContextSheetSizeView, height: Double) in
    ...
    Function("getShowcaseScene") { () -> String? in
      let scenePath = NSHomeDirectory() + "/Library/Caches/G2ShowcaseScene"
    ...
    AsyncFunction("applyShowcaseOrientation") { (orientation: String) in
      ...
      for case let windowScene as UIWindowScene in UIApplication.shared.connectedScenes {
        windowScene.requestGeometryUpdate(.iOS(interfaceOrientations: mask)) { error in
          NSLog("G2NativeControls applyShowcaseOrientation(\(orientation)) failed: \(error)")
        }
    ...
    Function("markShowcaseReady") { (scene: String) in
      let readyPath = NSHomeDirectory() + "/Library/Caches/G2ShowcaseReadyScene"
  ...
  private func presentVideo(url: URL, title: String, sourceIdentifier: String, identifier: String, promise: Promise) throws {
    ...
    guard videoPresentation == nil, filePresentation == nil,
      ...
      throw NSError(
        domain: "G2NativeVideo",
        code: 2,
        userInfo: [NSLocalizedDescriptionKey: "The video preview is no longer available."]
      )
    }
    let presentation = G2NativeVideoPresentation(identifier: identifier, url: url, title: title) { [weak self] error in
  ...
  private func presentFile(url: URL, title: String, sourceIdentifier: String,
    ...
    let file = G2NativeFilePresentation(identifier: identifier, sources: presentationSources,
