#import "G2MarkdownText.h"
#import "G2MarkdownTextShadowNode.h"
#import "G2MarkdownTextComponentDescriptor.h"
#import "G2MarkdownTextRun.h"
#import "G2ContextChip.h"
#import <React/RCTConversions.h>
#import <objc/runtime.h>

#import <react/renderer/textlayoutmanager/RCTAttributedTextUtils.h>
#import <react/renderer/components/G2MarkdownTextSpec/EventEmitters.h>
#import <react/renderer/components/G2MarkdownTextSpec/Props.h>
#import <react/renderer/components/G2MarkdownTextSpec/RCTComponentViewHelpers.h>
#import "RCTFabricComponentsPlugins.h"

using namespace facebook::react;

@interface G2ContextChipAccessibilityElement : UIAccessibilityElement
@property(nonatomic, weak) G2MarkdownTextRun *run;
@end

@implementation G2ContextChipAccessibilityElement
...
@interface G2ContextCopyTextView : UITextView
@property(nonatomic, copy) NSDictionary *contextClipboardConfig;
@end

@implementation G2ContextCopyTextView
...
- (void)copy:(id)sender
{
  ...
  if (encoded && copied.count > 0) {
    ...
    item[@"app.g2.context-fragment"] = encoded;
    item[@"public.html"] = [[NSString stringWithFormat:@"<pre data-g2-context-fragment=\"%@\">%@</pre>", attribute, escaped] dataUsingEncoding:NSUTF8StringEncoding];
  }
  UIPasteboard.generalPasteboard.items = @[item];
}
@end

static void G2MarkdownTextApplyParagraphStyles(
...
static void G2MarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    ...
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    attachment.bounds = CGRectMake(
        ...
        attachmentSize,
        attachmentSize);
    NSDictionary *chip = G2ContextChipPayload(imageUri);
    if (chip != nil) {
      CGSize size = CGSizeMake(attachmentRange.chipWidth, attachmentRange.chipHeight);
      attachment.bounds = G2ContextChipBounds(runAttributes[NSFontAttributeName], size);
      NSString *iconUri = [chip[@"iconUri"] isKindOfClass:NSString.class] ? chip[@"iconUri"] : nil;
      attachment.image = G2ContextChipImage(chip, size, iconUri ? images[iconUri] : nil);
    }
...
@protocol G2MarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface G2MarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<G2MarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<G2MarkdownOutsideTapTarget>)target;

@end

static const void *G2MarkdownOutsideTapCoordinatorKey =
    &G2MarkdownOutsideTapCoordinatorKey;

@implementation G2MarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<G2MarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<G2MarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, G2MarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<G2MarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static G2MarkdownOutsideTapCoordinator *
G2MarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  G2MarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, G2MarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[G2MarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        G2MarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface G2MarkdownText () <RCTT3MarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface G2MarkdownText () <G2MarkdownOutsideTapTarget>
@end

@implementation G2MarkdownText {
  UIView * _view;
  G2ContextCopyTextView * _textView;
  G2MarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<G2MarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const G2MarkdownTextProps>();
    _props = defaultProps;
    ...
    _textView = [[G2ContextCopyTextView alloc] init];
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    G2MarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, G2MarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [G2MarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<G2MarkdownTextProps const>(_props);
  ...
  G2MarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  G2MarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  for (UIView *child in self.subviews) {
    if (![child isKindOfClass:[G2MarkdownTextRun class]]) {
      continue;
    }

    G2MarkdownTextRun *textChild = (G2MarkdownTextRun *)child;
    ...
    NSURL *link = [NSURL URLWithString:
        [NSString stringWithFormat:@"g2-markdown-run://%ld", (long)textChild.tag]];
    if (link != nil) {
  ...
  if (textChanged) {
    ...
    for (auto it = _state->getData().attachmentRanges.rbegin();
      ...
      NSDictionary *chip = G2ContextChipPayload([NSString stringWithUTF8String:it->imageUri.c_str()]);
  ...
  for (UIView *child in self.subviews) {
    if (![child isKindOfClass:G2MarkdownTextRun.class]) continue;
    G2MarkdownTextRun *run = (G2MarkdownTextRun *)child;
    run.contextChipInteractive = NO;
  }
  for (const auto &attachmentRange : _state->getData().attachmentRanges) {
    NSDictionary *chip = G2ContextChipPayload(
    ...
    G2MarkdownTextRun *run = [self childForCharacterRange:range];
    ...
    G2ContextChipAccessibilityElement *element =
        [[G2ContextChipAccessibilityElement alloc] initWithAccessibilityContainer:self];
    element.run = run;
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::G2MarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::G2MarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<G2MarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
...
{
  const auto &oldViewProps = *std::static_pointer_cast<G2MarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<G2MarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == G2MarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == G2MarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == G2MarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == G2MarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const G2MarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (nullable G2MarkdownTextRun *)childForCharacterRange:(NSRange)characterRange
{
...
- (G2MarkdownTextRun*)getTouchChild:(CGPoint)location
{
...
{
  G2MarkdownTextRun *child = [self childForCharacterRange:textItem.range];
  if (![child hasContextMenu] && !child.contextChipInteractive) {
    return defaultAction;
  }

  __weak G2MarkdownTextRun *weakChild = child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::G2MarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::G2MarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> G2MarkdownTextCls(void)
{
  return G2MarkdownText.class;
}
