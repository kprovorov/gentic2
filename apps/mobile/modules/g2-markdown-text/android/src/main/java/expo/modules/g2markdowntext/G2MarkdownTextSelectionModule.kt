package expo.modules.g2markdowntext
...
private fun contextClipData(selectedText: String, fragment: String): ClipData {
  ...
  return ClipData.newHtmlText(
    null,
    selectedText,
    "<pre data-g2-context-fragment=\"$attribute\">$escaped</pre>"
  )
}
...
class G2MarkdownTextSelectionModule : Module() {
    ...
    Function("renderContextChip") { payloadJson: String ->
      ...
      val chip = G2ContextChip(
        ...
        colors = G2ContextChip.Colors(
          accent = G2ContextChip.color(payload.optString("accent"), Color.GRAY),
          foreground = G2ContextChip.color(payload.optString("foreground"), Color.BLACK),
          border = G2ContextChip.color(payload.optString("border"), Color.GRAY)
        ),
