...
import { decodeComposerContextFragment } from "@gentic2/shared/composerContextClipboard";
import {
  findNodeHandle,
  Image,
  Linking,
...
} from "./G2MarkdownTextSelectionModule";
...
export function NativeMarkdownSelectableText(props: {
  ...
  // G2MarkdownText only rebuilds its attributed string during native layout. A
        ...
        return (
          <MarkdownTextPrimitive
            ...
            nativeID={
              Platform.OS === "ios"
                ? chip
                  ? `g2-chip:${JSON.stringify(chip)}`
                  : run.fileIcon
                    ? `g2-file:${Image.resolveAssetSource(markdownFileIconSource(run.fileIcon)).uri}`
                    : run.skillName
                      ? "g2-skill:sf:cube"
                      : linkIcon
                        ? `g2-link:${Image.resolveAssetSource(markdownLinkIconSource(linkIcon)).uri}`
                        : undefined
                : undefined
            }
