...
> [!WARNING]
> Gentic2 Mobile is currently in development and is not distributed yet. If you want to try it out, you can build it from source.
...
- `development`: Expo dev client, installable side-by-side as `Gentic2 Dev`
- `preview`: persistent internal preview build, installable side-by-side as `Gentic2 Preview`
- `production`: store/release build as `Gentic2`

Run commands from `apps/mobile`.

Gentic2 Connect is optional and disabled in a fresh clone. Public configuration belongs in the
...
```bash
GENTIC2_IOS_PERSONAL_TEAM=1 \
GENTIC2_IOS_PERSONAL_TEAM_BUNDLE_ID=com.example.gentic2.dev \
vp run ios:dev
```
...
```bash
GENTIC2_IOS_PERSONAL_TEAM=1 \
GENTIC2_IOS_PERSONAL_TEAM_BUNDLE_ID=com.example.gentic2 \
vp run ios:release
```
...
For preview or production EAS environments, set `GENTIC2_CLERK_PUBLISHABLE_KEY`,
`GENTIC2_CLERK_JWT_TEMPLATE`, and `GENTIC2_RELAY_URL`
