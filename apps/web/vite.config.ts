...
import { DEV_PROXIED_PATH_PREFIXES } from "@gentic2/shared/devProxy";
...
const isSingleOriginDev = process.env.GENTIC2_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_GENTIC2_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.GENTIC2_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: GENTIC2_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); GENTIC2_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.GENTIC2_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // GENTIC2_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.GENTIC2_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.GENTIC2_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_GENTIC2_RELAY_URL": JSON.stringify(configuredRelayUrl),
