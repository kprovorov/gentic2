import type { EnvironmentId, ServerSelfUpdateCapability } from "@gentic2/contracts";
import type { ServerUpdateStage, ServerUpdateState } from "@gentic2/client-runtime/state/server";
import {
      ...
      toastManager.add({
        ...
        description:
          selfUpdate === "desktop-managed"
            ...
            : `Reconnected on g2@${result.value.targetVersion}.`,
      });
    } catch (error) {
      toastManager.add({
        type: "error",
      ...
      if (desktopTargets.length > 0) {
        const confirmed =
          (await requestConfirmDialog(
            `Update the Gentic2 desktop apps on ${desktopTargets.map((target) => target.serverLabel).join(", ")}? They will close and relaunch on those machines.`,
          )) ?? true;
        if (!confirmed) return;
      }
      await Promise.all(
...
export function ServerUpdateAction({
  ...
  const handleUpdate = async () => {
    ...
    if (isDesktopAppUpdate) {
      ...
      const confirmed =
        (await requestConfirmDialog(
          `Update the Gentic2 desktop app that runs the ${serverLabel}? It will close and relaunch on that machine.`,
        )) ?? true;
      if (!confirmed) {
        return;
      }
