...
import { CommandId, ProviderDriverKind, ThreadId } from "@gentic2/contracts";
...
import { useG2ConnectAuthPrompt } from "../clerk/useG2ConnectAuthPrompt";
...
export function WelcomeWizard({
  ...
  return (
    <Dialog open disablePointerDismissal onOpenChange={(_, event) => event.cancel()}>
      ...
      >
        <WizardHeader
          title="Set up Gentic2"
          identity={
            <div className="flex items-baseline gap-1.5" role="img" aria-label="Gentic2">
              <G2Wordmark className="h-4 w-auto shrink-0" aria-hidden />
...
function ConnectAccountOption({
  ...
  const { openAuthPrompt } = useG2ConnectAuthPrompt();
    ...
    <div className="rounded-lg border border-border bg-background">
      <Collapsible open={expanded && !!isSignedIn && discoveryReady} onOpenChange={setExpanded}>
        ...
        >
          ...
          <span className="flex-1 text-left">Gentic2 Connect</span>
        ...
        <CollapsiblePanel keepMounted>
          <div className="px-3 pb-3">
            ...
            <CommandBlock command="npx g2 connect" className="mt-3" />
            <p className="mt-3 text-xs text-muted-foreground">
              Keep Gentic2 running. Select the computers you want to set up above.
            </p>
          </div>
        </CollapsiblePanel>
      </Collapsible>
      ...
      >
        ...
        <Collapsible>
          ...
          <CollapsiblePanel>
            ...
            <CommandBlock command="npx g2 pair" className="mt-2" />
            <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
              Start Gentic2 first, or run <code className="font-mono">npx g2 serve</code>. Add{" "}
