...
import type { BrowserImportSource } from "@gentic2/contracts";
import { BROWSER_IMPORT_FAILURE_COPY } from "@gentic2/contracts";
...
function FullDiskAccessStep({
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>Let Gentic2 read {source.name}&rsquo;s cookies</DialogTitle>
        <DialogDescription>
          To import cookies from {source.name}, Gentic2 needs Full Disk Access. Turn it on in System
      ...
      <DialogPanel>
        ...
        {!permission.isReady(["fullDiskAccess"]) ? (
          <p className="mt-3 text-xs text-muted-foreground">
            {stillRequired
              ? "Access is still required. Quit and reopen Gentic2 if you just allowed it, then retry the import."
              : "If access doesn't update after you allow it, quit and reopen Gentic2, then retry the import."}
          </p>
        ) : null}
      </DialogPanel>
      <DialogFooter>
