...
describe("fileBreadcrumbs", () => {
  it("builds project, directory, and file crumbs", () => {
    expect(fileBreadcrumbs("gentic2", "apps/web/src/main.tsx")).toEqual([
      { label: "gentic2", path: "", kind: "project" },
  ...
  it("starts host paths outside the workspace at the filesystem root", () => {
    expect(fileBreadcrumbs("gentic2", "/tmp/g2-cleanup/report.md")).toEqual([
      { label: "tmp", path: "/tmp", kind: "directory" },
      { label: "g2-cleanup", path: "/tmp/g2-cleanup", kind: "directory" },
      { label: "report.md", path: "/tmp/g2-cleanup/report.md", kind: "file" },
    ]);
    expect(fileBreadcrumbs("gentic2", "C:\\Temp\\report.md")).toEqual([
      { label: "C:", path: "C:", kind: "directory" },
      { label: "Temp", path: "C:\\Temp", kind: "directory" },
      { label: "report.md", path: "C:\\Temp\\report.md", kind: "file" },
    ]);
    expect(fileBreadcrumbs("gentic2", "\\\\server\\share\\report.md").map((c) => c.path)).toEqual([
      "\\\\server",
      "\\\\server\\share",
