...
describe("Pierre file icons", () => {
  ...
  it("extends Pierre with G2-specific exact filename icons", () => {
    assert.equal(resolvePierreIconForEntry("AGENTS.md", "file")?.name, "g2-file-icon-agents");
    assert.equal(resolvePierreIconForEntry("pnpm-lock.yaml", "file")?.name, "g2-file-icon-pnpm");
    assert.equal(
  ...
  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(
      Object.values(G2_PIERRE_ICONS.byFileName).filter((name) => name.startsWith("g2-")),
    );
    for (const iconName of customIconNames) {
      assert.include(G2_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });
