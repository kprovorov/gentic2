...
describe("composerDraftStore terminal contexts", () => {
  ...
  it("keeps legacy terminal contexts without saved snapshot text unavailable", () => {
    ...
    expect(mergedState.draftsByThreadKey[threadKeyFor(threadId)]?.prompt).toBe(
      "[Terminal 1 lines 4-5](g2-context://v1/terminal/terminal_ctx-rehydrated)",
    );
...
describe("composerDraftStore context persistence", () => {
  ...
  it("preserves snapshots and inline positions through repeated persistence and hydration", () => {
    ...
    const prompt = `Inspect ${terminalLink}. Apply [Retry note](g2-context://v1/preview-annotation/preview-annotation_retry-note). Compare ${terminalLink} again.`;
  ...
  it("retains annotation-only drafts and filters malformed annotations", () => {
    ...
    expect(draft?.prompt).toContain(
      "g2-context://v1/preview-annotation/preview-annotation_retry-note",
    );
  });
});
...
describe("composerDraftStore inline context references", () => {
  ...
  const reviewLink = "[b.ts L4](g2-context://v1/review-comment/review-comment_rc-1)";
  const annotation = {
    id: "ann-1",
  ...
  const annotationLink = "[Bigger](g2-context://v1/preview-annotation/preview-annotation_ann-1)";
  ...
  it("hands new review comments to a registered caret handler instead of appending", () => {
    ...
    expect(draftFor(threadId, TEST_ENVIRONMENT_ID)?.prompt).toBe(
      "look [b.ts L4](g2-context://v1/review-comment/review-comment_rc-2) ",
    );
  });
  ...
  it("does not overwrite a prompt update made by the caret handler", () => {
    ...
    store.setContextInsertionHandler(threadRef, (references) => {
      ...
      useComposerDraftStore
        ...
        .setPrompt(
          threadRef,
          `before [${reference.label}](g2-context://v1/${reference.kind}/${reference.contextId}) after`,
        );
      return true;
    });
...
describe("composerDraftStore attachment references", () => {
  ...
  const threadRef = scopeThreadRef(TEST_ENVIRONMENT_ID, threadId);
  const fileLink = "[notes.txt](g2-context://v1/file/file_file-1)";
  const imageLink = "[shot.png](g2-context://v1/image/image_img-1)";
  ...
  it("moves a needs-reattach marker's chip to the re-picked file", () => {
    const store = useComposerDraftStore.getState();
    store.setPrompt(threadRef, "see [notes.txt](g2-context://v1/file/file_marker-1) ok");
    store.addFiles(threadRef, [
      {
        type: "file",
        id: "marker-1",
    ...
    expect(draft?.prompt).toBe("see [notes.txt](g2-context://v1/file/file_fresh-1) ok");
  });
  ...
  it("restores a file-only draft with a reference and preserves it through prompt edits", () => {
    ...
    expect(restored.prompt).toBe("[notes.txt](g2-context://v1/file/file_restored) ");
    ...
    expect(edited.prompt).toContain("g2-context://v1/file/file_restored");
  });
  ...
  it.each(["old.file:1", "file-1"])(
    ...
    (id) => {
      const merged = useComposerDraftStore.persist.getOptions().merge!(
        {
          draftsByThreadKey: {
            [threadKeyFor(threadId, TEST_ENVIRONMENT_ID)]: {
              prompt: `before [notes.txt](g2-context://v1/file/${id}) after`,
              attachments: [],
      ...
      const draft = draftFor(threadId, TEST_ENVIRONMENT_ID)!;
      expect(draft.prompt.match(/g2-context:/g)).toHaveLength(1);
  ...
  it.each(["", "Fix this"])("migrates saved element context with prompt %j", (prompt) => {
    ...
    expect(draft.prompt).toContain("g2-context://v1/preview-annotation/");
  ...
  it("migrates legacy image references without doubling their image marker", () => {
    const merged = useComposerDraftStore.persist.getOptions().merge!(
      {
        draftsByThreadKey: {
          [threadKeyFor(threadId, TEST_ENVIRONMENT_ID)]: {
            prompt: "see ![shot.png](g2-context://v1/image/img-1) after",
            attachments: [
              {
                id: "img-1",
                name: "shot.png",
    ...
    expect(merged.draftsByThreadKey[threadKeyFor(threadId, TEST_ENVIRONMENT_ID)]?.prompt).toBe(
      "see ![shot.png](g2-context://v1/image/image_img-1) after",
    );
  });

  it("preserves canonical references when another producer ID matches their namespace", () => {
    const prompt =
      "![x.png](g2-context://v1/image/image_x) ![image_x.png](g2-context://v1/image/image_image_x)";
    const merged = useComposerDraftStore.persist.getOptions().merge!(
      {
        draftsByThreadKey: {
          [threadKeyFor(threadId, TEST_ENVIRONMENT_ID)]: {
