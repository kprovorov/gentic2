...
describe("terminalLinkAtPositionWithRange", () => {
  it("maps terminal cells to UTF-16 offsets after a wide emoji", () => {
    const cells = [
      cell("🙂"),
      cell(""),
      ...Array.from("https://gentic2.com", (character) => cell(character)),
    ];
    ...
    expect(terminalLinkAtPositionWithRange([row], 0, 2)?.text).toBe("https://gentic2.com");
  ...
  it("refuses links truncated at the viewport edges instead of mis-resolving", () => {
    ...
    const tailCut = [row("https://gentic2.com", false, true)];
    expect(terminalLinkAtPositionWithRange(tailCut, 0, 8)).toBeNull();
    // A partial bottom row is provably complete and still resolves.
    const complete = [row("https://gentic2.com", false), row("", false)];
    expect(terminalLinkAtPositionWithRange(complete, 0, 8)?.text).toBe("https://gentic2.com");
    ...
    const wideFull: GhosttyRow = {
      cells: [
        { ...cell("🙂"), wide: 1 },
        { ...cell(""), wide: 2 },
        ...Array.from("https://g2.code", (character) => cell(character)),
      ],
      text: "🙂 https://g2.code",
      isWrapContinuation: false,
      wrapsToNext: true,
