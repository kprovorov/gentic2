...
import { EnvironmentRegistry } from "@gentic2/client-runtime/connection";
import { request, runStream } from "@gentic2/client-runtime/rpc";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Gentic2 cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Gentic2 cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
// independently of Gentic2 Connect while clients reach the environment out of band.
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "GENTIC2_RELAY_URL is not configured.",
      });
    }
