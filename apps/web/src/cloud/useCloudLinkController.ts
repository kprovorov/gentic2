...
/**
 * Drives the primary environment's Gentic2 Connect link. Gentic2 Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Gentic2 Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[g2-connect] Could not update Gentic2 Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update Gentic2 Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Gentic2 Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Gentic2 Connect before enabling this."));
        return false;
      }
