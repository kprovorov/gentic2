import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@gentic2/contracts";
import type { ServerUpdateState } from "@gentic2/client-runtime/state/server";
...
const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "gentic2:version-mismatch-dismissals:v1";
...
/**
 * The skew a user can act on: the connected server runs an older Gentic2 than
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same Gentic2 version.",
  };
}
