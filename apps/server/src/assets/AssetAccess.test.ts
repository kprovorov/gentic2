...
import { AssetAccessError, AssetPreviewTypeValidationError, ThreadId } from "@gentic2/contracts";
import { PROJECT_FAVICON_FALLBACK_MARKER } from "@gentic2/shared/projectFavicon";
...
describe("AssetAccess", () => {
  ...
  it.effect("issues exact URLs for media and browser documents outside the workspace", () =>
    Effect.gen(function* () {
      ...
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "g2-media-root-" });
      const outside = yield* fs.makeTempDirectoryScoped({ prefix: "g2-media-outside-" });
  ...
  it.effect("reports pixel dimensions from an image header and nothing for other files", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "g2-media-dimensions-" });
  ...
  it.effect("resolves relative media paths from the thread workspace, including outside it", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "g2-media-relative-" });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "g2-media-validation-" });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "g2-media-symlink-" });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "g2-media-open-file-" });
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "g2-media-open-race-" });
        const filePath = path.join(root, "recording.mp4");
        const secretPath = path.join(root, "secret.txt");
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "g2-media-open-rejected-" });
        const filePath = path.join(root, "recording.mp4");
        const secretPath = path.join(root, "secret.txt");
        yield* fs.writeFileString(filePath, "video");
        yield* fs.writeFileString(secretPath, "secret");
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "g2-media-parent-race-" });
  ...
  it.effect("keeps in-place edits readable but requires a new URL after atomic replacement", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "g2-media-replacement-" });
      const filePath = path.join(root, "recording.mp4");
  ...
  it.effect("rejects workspace files outside the authorized root", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "g2-asset-root-",
      });
      const outside = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "g2-asset-outside-",
      });
  ...
  it.effect("serves absolute draft media files exactly, wherever they live", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "g2-asset-draft-root-",
      });
      const outside = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "g2-asset-draft-outside-",
      });
  ...
  it.effect("falls back to the resource cwd for relative draft paths", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "g2-asset-draft-fallback-",
      });
      const htmlPath = path.join(root, "report.html");
  ...
  it.effect("preserves non-missing canonical path failures when issuing asset URLs", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "g2-asset-permission-root-",
      });
      const htmlPath = path.join(root, "report.html");
  ...
  it.effect("issues exact workspace URLs for image previews", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "g2-asset-image-workspace-",
      });
  ...
  it.effect("issues project favicon capabilities for a saved override", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "g2-asset-favicon-override-",
      });
  ...
  it.effect("issues an exact capability for a saved favicon outside the workspace", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "g2-asset-favicon-workspace-",
      });
      const pictures = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "g2-asset-favicon-pictures-",
      });
  ...
  it.effect("keeps automatic favicon resolution separate from a saved override", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "g2-asset-favicon-automatic-",
      });
      yield* fileSystem.makeDirectory(path.join(root, "brand"));
      yield* fileSystem.writeFileString(path.join(root, "brand", "saved.svg"), "<svg>saved</svg>");
  ...
  it.effect("buckets project favicon expiry after content hashing", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "g2-asset-favicon-expiry-",
      });
