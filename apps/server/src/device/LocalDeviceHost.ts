...
import { deviceToolInstallMessage } from "@gentic2/contracts";
/**
 * The device host that is this machine.
 *
 * Runs expo-device-hub as a supervised child on a loopback port and starts the
 * agent-device daemon in HTTP mode under a G2-owned state directory. Both are
 ...
 *
 * The hub runs in its standalone mode (origin root). The G2 proxy strips its
