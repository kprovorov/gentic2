...
const status = {
  ...
  installed: true,
  current: true,
  unitPath: "/home/me/.config/systemd/user/gentic2.service",
  logPath: "/home/me/.g2/userdata/logs/boot-service.log",
} as const;
...
it("reports the installed service version and host paths", () => {
  assert.equal(
    ...
    [
      "Gentic2 service",
      "  Status: installed · g2@0.0.29",
      "  Unit: /home/me/.config/systemd/user/gentic2.service",
      "  Logs: /home/me/.g2/userdata/logs/boot-service.log",
    ].join("\n"),
  );
});
...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `g2 service install` to repair it.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("Run `g2 service install` to repair it.");
...
it("reports a newer installed service and tells the CLI to catch up to it", () => {
  ...
  assert.include(output, "g2@0.0.32-nightly.1 (newer than this g2@0.0.31 CLI)");
  assert.include(output, "Run `g2 update 0.0.32-nightly.1` to match it");
  ...
  const service = BootService.BootService.of({
    ...
    install: (options) =>
      Effect.sync(() => {
        ...
        return {
          program: ["/test/g2/runtime/versions/1.0.0/g2", "__service-launcher"],
          baseDir: "/test/g2",
...
it.layer(Layer.mergeAll(NodeServices.layer, NetService.layer))("service commands", (it) => {
  it.effect("restart restarts the installed service", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "g2-service-cli-test-" });
