...
import type { DesktopAppActivationRequest } from "@gentic2/contracts";
...
describe("g2 app", () => {
  it.effect("rejects SSH before it tries to reach a desktop app", () =>
    withTempDirectory("g2-app-ssh-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-g2-home");
        ...
        expect(error).toMatchObject({
          ...
          message:
            "`g2 app` only controls a desktop app on the same machine. It cannot run over SSH.",
        });
  ...
  it.effect("rejects unsupported platforms without creating state", () =>
    withTempDirectory("g2-app-platform-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-g2-home");
        ...
        expect(error).toMatchObject({
          ...
          platform: "freebsd",
          message: "`g2 app` is not supported on freebsd.",
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
    ),
  ...
  it.effect("does not create state when only a server or no desktop app is running", () =>
    withTempDirectory("g2-app-missing-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-g2-home");
        ...
        expect(error).toMatchObject({
          ...
          message: expect.stringContaining("Could not reach the Gentic2 desktop app."),
          cause: { code: "ENOENT" },
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
  ...
  it.effect("uses GENTIC2_HOME or --base-dir and sends the default or explicit path", () =>
    withTempDirectory("g2-app-command-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "g2-home");
        ...
        yield* runCli(["app"], { GENTIC2_HOME: baseDir });
  ...
  it.effect("prefers the installed desktop app when a dev desktop is also running", () =>
    withTempDirectory("g2-app-preferred-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".g2");
        const desktop = yield* fakeDesktop({ baseDir });
  ...
  it.effect("finds the dev desktop when the default desktop socket is absent", () =>
    withTempDirectory("g2-app-dev-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".g2");
        const development = yield* fakeDesktop({ baseDir, stateSubdirectory: "dev" });
  ...
  it.effect("never searches a dev state directory for an explicit Gentic2 home", () =>
    withTempDirectory("g2-app-explicit-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".g2");
        const development = yield* fakeDesktop({ baseDir, stateSubdirectory: "dev" });
        ...
        const envError = yield* runCli(["app"], { GENTIC2_HOME: baseDir }).pipe(Effect.flip);
  ...
  for (const responseKind of ["failure", "invalid"] as const) {
    it.effect(`never falls back after the default desktop sends a ${responseKind} response`, () =>
      withTempDirectory("g2-app-response-test-", (root) =>
        Effect.gen(function* () {
          vi.mocked(NodeOS.homedir).mockReturnValue(root);
          const baseDir = NodePath.join(root, ".g2");
