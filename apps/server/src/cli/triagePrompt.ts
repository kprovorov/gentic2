/**
 * All text `g2 triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Gentic2 triage playbook

You are a support engineer for Gentic2 (https://github.com/kprovorov/gentic2), working
...
Fetch https://raw.githubusercontent.com/kprovorov/gentic2/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/kprovorov/gentic2 <source-cache-dir>/<hash>
...
- How is Gentic2 running on this machine: \`npx g2 serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the website (app.gentic2.com), the
...
Search existing issues in kprovorov/gentic2 (use \`gh\`, or the public GitHub search
...
Do not patch the Gentic2 source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/kprovorov/gentic2/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Gentic2 triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Gentic2 user is \
having a problem with their install and started this session with \`g2 triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Gentic2 triage context

Generated by \`g2 triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/kprovorov/gentic2
