...
import { DesktopBackendBootstrap, PortSchema } from "@gentic2/contracts";
...
export const baseDirFlag = Flag.String("base-dir").pipe(
  Flag.withDescription(
    "Explicit Gentic2 data directory; runtime state is stored under userdata (equivalent to GENTIC2_HOME).",
  ),
  Flag.optional,
);
...
const logWebSocketEventsFlag = Flag.Boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to GENTIC2_LOG_WS_EVENTS).",
  ),
...
const EnvServerConfig = Config.all({
  logLevel: Config.LogLevel("GENTIC2_LOG_LEVEL").pipe(Config.withDefault("Info")),
  traceMinLevel: Config.LogLevel("GENTIC2_TRACE_MIN_LEVEL").pipe(Config.withDefault("Info")),
  traceTimingEnabled: Config.Boolean("GENTIC2_TRACE_TIMING_ENABLED").pipe(Config.withDefault(true)),
  traceFile: Config.String("GENTIC2_TRACE_FILE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  traceMaxBytes: Config.Int("GENTIC2_TRACE_MAX_BYTES").pipe(Config.withDefault(10 * 1024 * 1024)),
  traceMaxFiles: Config.Int("GENTIC2_TRACE_MAX_FILES").pipe(Config.withDefault(10)),
  traceBatchWindowMs: Config.Int("GENTIC2_TRACE_BATCH_WINDOW_MS").pipe(Config.withDefault(1_000)),
  otlpTracesUrl: Config.String("GENTIC2_OTLP_TRACES_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpMetricsUrl: Config.String("GENTIC2_OTLP_METRICS_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpLogsUrl: Config.String("GENTIC2_OTLP_LOGS_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  ...
  otlpServiceName: Config.String("GENTIC2_OTLP_SERVICE_NAME").pipe(Config.withDefault("g2-server")),
  otlpHeaders: Config.schema(OtlpHeadersFromString, "GENTIC2_OTLP_HEADERS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  ...
  mode: Config.schema(ServerConfig.RuntimeMode, "GENTIC2_MODE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  port: Config.Port("GENTIC2_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.String("GENTIC2_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  g2Home: Config.String("GENTIC2_HOME").pipe(Config.option, Config.map(Option.getOrUndefined)),
  devUrl: Config.URL("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  devAllowedOrigins: Config.String("GENTIC2_DEV_ALLOWED_ORIGINS").pipe(
    Config.withDefault(""),
    Config.map((value) =>
      value
        .split(",")
  ...
  noBrowser: Config.Boolean("GENTIC2_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  bootstrapFd: Config.Int("GENTIC2_BOOTSTRAP_FD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.Boolean("GENTIC2_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.Boolean("GENTIC2_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  tailscaleServeEnabled: Config.Boolean("GENTIC2_TAILSCALE_SERVE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  tailscaleServePort: Config.Port("GENTIC2_TAILSCALE_SERVE_PORT").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});

const DevAuthTokenConfig = Config.Redacted("GENTIC2_DEV_AUTH_TOKEN").pipe(
          ...
          new Config.ConfigError(
            new Schema.SchemaError(
              new SchemaIssue.InvalidValue({
                message: "GENTIC2_DEV_AUTH_TOKEN must contain at least 32 characters.",
              }),
            ),
          ),
        ),
...
export const resolveServerConfig = (
  ...
  Effect.gen(function* () {
    ...
    const explicitBaseDir = resolveOptionPrecedence(
      ...
      Option.fromUndefinedOr(env.g2Home),
    ).pipe(Option.filter((value) => value.trim().length > 0));
    const baseDir = yield* resolveBaseDir(
      Option.getOrUndefined(
        resolveOptionPrecedence(explicitBaseDir, Option.fromUndefinedOr(bootstrap?.g2Home)),
      ),
    );
    ...
    // Gentic2's own OTLP variables name no signal, so the one answer they give
