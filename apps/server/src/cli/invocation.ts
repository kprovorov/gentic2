...
import { HostProcessArguments } from "@gentic2/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `g2` command.
 */
...
/**
 * The `g2` package spec to suggest. The literal spec the user typed (e.g.
 * `g2@nightly`) is resolved away before our process starts, so re-derive it
...
function suggestedPackageSpec(version: string): string {
  ...
  return channel === undefined ? "g2" : `g2@${channel}`;
}

/**
 * Render a `g2 <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx g2 connect` suggests
 * `npx g2 serve`, a global install suggests `g2 serve`, and a nightly build
