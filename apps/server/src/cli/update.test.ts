...
it.layer(NodeServices.layer)("g2 update launcher", (it) => {
  it.effect("repoints a symlink that lives in a runtime versions tree", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "g2-update-" });
      const oldExe = path.join(root, "runtime/versions/1.0.0/g2");
      const newExe = path.join(root, "runtime/versions/2.0.0/g2");
      const launcher = path.join(root, "bin/g2");
  ...
  it.effect("leaves a plain copy or a foreign symlink alone", () =>
    Effect.gen(function* () {
      ...
      const foreign = path.join(root, "foreign/g2");
      const elsewhere = path.join(root, "elsewhere/g2");
      // Another install's versions tree: same shape, different home.
      const otherHome = path.join(root, "other/runtime/versions/1.0.0/g2");
      const otherLauncher = path.join(root, "other/bin/g2");
  ...
  it.effect("finds the launcher a bare command name resolved to on PATH", () =>
    Effect.gen(function* () {
      ...
      const bare = yield* resolveLauncherPath.pipe(
        Effect.provideService(HostProcessInvokedAs, "g2"),
      ...
      const relative = yield* resolveLauncherPath.pipe(
        Effect.provideService(HostProcessInvokedAs, "./bin/g2"),
