  ...
  for (let page = 1; page <= RELEASE_INDEX_MAX_PAGES; page += 1) {
    const body = yield* httpClient
      ...
      .pipe(
        ...
        Effect.mapError(() => new CliUpdateError({ reason: "Could not list g2 releases." })),
        Effect.timeoutOrElse({
          duration: RELEASE_INDEX_TIMEOUT,
          orElse: () =>
            Effect.fail(new CliUpdateError({ reason: "Timed out listing g2 releases." })),
        }),
      );
    const releases = yield* decodeReleaseIndex(body).pipe(
      Effect.mapError(
        () => new CliUpdateError({ reason: "The g2 release index had an unexpected shape." }),
      ),
    );
...
/**
 * The launcher the install scripts leave behind: a symlink at `<bin>/g2` on
 * POSIX, a `g2.cmd` shim on Windows. `g2 update` repoints it so the next `g2`
...
export const repointLauncher = Effect.fn("cli.update.repoint_launcher")(function* (input: {
  ...
  if (platform === "win32") {
    ...
    // itself as argv0; the shim is the `g2.cmd` next to it only when launched
    // from an install script's bin directory. Find it by searching the
    // directories that would resolve `g2` on this shell's PATH.
    ...
    yield* fs
      ...
      .pipe(
        Effect.mapError(
          () => new CliUpdateError({ reason: `Could not rewrite the g2 launcher at ${shimPath}.` }),
        ),
      );
  ...
  yield* fs.symlink(input.targetEntryPath, tempLink).pipe(
    ...
    Effect.mapError(
      () =>
        new CliUpdateError({ reason: `Could not repoint the g2 launcher at ${input.launchedAs}.` }),
    ),
  );
 ...
 * spelling in argv0: a launcher symlink or `./g2` resolves against the
 * working directory, while a bare `g2` was found on PATH and has to be
 ...
 * only ever sees its own path. Walk PATH for a `g2.cmd` whose target is the
...
export const findWindowsShim = Effect.fn("cli.update.find_windows_shim")(function* (
  ...
  const environment = yield* HostProcessEnvironment;
  const candidates = [
    ...(environment["GENTIC2_INSTALL_BIN_DIR"] ? [environment["GENTIC2_INSTALL_BIN_DIR"]] : []),
    ...(environment["PATH"] ?? environment["Path"] ?? "").split(";"),
  ].filter((entry) => entry.trim().length > 0);
  for (const directory of candidates) {
    const shimPath = path.join(directory, "g2.cmd");
...
const updateFlags = {
  ...
  channel: Flag.Literals("channel", CLI_RELEASE_CHANNELS).pipe(
    Flag.withDescription(
      "Release channel to follow. Defaults to the channel this g2 was published on.",
    ),
    Flag.optional,
  ),
...
export const updateCommand = Command.make("update", {
  ...
  Command.withDescription(
    "Download a newer g2 and switch this machine to it, including the background service when one is installed.",
  ),
...
/**
 * A `g2 serve` or `g2` someone started by hand, as opposed to the one the
...
const belongsToBootService = Effect.fn("cli.update.belongs_to_boot_service")(function* (
  ...
  if (platform === "linux") {
    ...
    return Option.isSome(cgroup) && cgroup.value.includes("/gentic2.service");
  }
  if (platform === "darwin") {
...
const runUpdate = Effect.fn("cli.update.run")(function* (input: {
  ...
  if (input.requestedVersion !== undefined && !isExactServiceVersion(input.requestedVersion)) {
    return yield* new CliUpdateError({
      reason: `'${input.requestedVersion}' is not an exact g2 version.`,
    });
  }
  ...
  if (targetChannel === "preview" && currentChannel !== "preview") {
    yield* Console.log(
      [
        `g2@${targetVersion} is a preview build.`,
        ...
        `  will have to switch back to ${currentChannel} yourself with \`g2 update --channel ${currentChannel} --allow-downgrade\`.`,
      ].join("\n"),
    );
  ...
  // The unit name is per user, not per Gentic2 home. Only touch the service when it
  ...
  if (executableCurrent && serviceCurrent) {
    yield* Console.log(
      serviceVersion !== undefined
        ? `g2 and its background service are already on ${targetVersion} (${targetChannel}).`
        : `g2 is already on ${targetVersion} (${targetChannel}).`,
    );
    return;
  }
  if (!input.allowDowngrade && compareExactServiceVersions(targetVersion, newestInstalled) < 0) {
    return yield* new CliUpdateError({
      reason: `g2@${targetVersion} is older than the installed ${newestInstalled}. Pass --allow-downgrade to install it anyway.`,
    });
  }
  ...
  if (serviceInstalled && !serviceCurrent) {
    yield* Console.log(
      "  A background service is installed for this Gentic2 home. Restarting it interrupts anything running in it: agent turns, terminals, remote clients.",
    );
    if (input.assumeYes) {
      restartService = true;
      ...
      yield* Console.log(
        "  Not a terminal, so the service keeps running its current version. Rerun with --yes to restart it now, or run `g2 service restart` later.",
      );
    }
  }
  ...
  const runtime = yield* ensurePinnedRuntimeInstalled({
    ...
    validate: (paths) =>
      runner
        ...
        .pipe(
          Effect.mapError(
            (cause) =>
              new PinnedRuntimeInstallError({ step: "verifying the downloaded g2", cause }),
          ),
            ...
            result.code === 0 && /\bv(\S+)\s*$/.exec(result.stdout)?.[1] === targetVersion
              ...
              : Effect.fail(
                  new PinnedRuntimeInstallError({
                    step: "verifying the downloaded g2",
    ...
    Effect.catchIf(
      (error): error is PinnedRuntimeInstallError =>
        ...
        error.step.startsWith("downloading the g2 release checksums") &&
        String(error.cause).includes("404"),
      () =>
        Effect.fail(
          new CliUpdateError({
            reason: `No release archive was published for g2@${targetVersion}.`,
          }),
        ),
    ),
  );
  ...
  // is rewritten either way so a later `g2 service restart` lands on the new
  ...
  if (serviceInstalled && !serviceCurrent) {
    ...
    yield* BootService.BootService.pipe(
      ...
      Effect.mapError(
        (error) =>
          new CliUpdateError({
            reason: `g2@${targetVersion} is installed but the background service could not be ${restartService ? "updated" : "pointed at it"}: ${error.message}`,
          }),
      ),
    );
  ...
  progress.success(`Installed Gentic2 ${targetVersion}`);
  if (Option.isSome(repointed)) {
    yield* Console.log("  Run g2 to get started.\n");
  } else {
  ...
  if (serviceUpdated) {
    ...
    yield* Console.log(
      `  Background service still running ${serviceVersion ?? "an unknown version"}. Run \`g2 service restart\` when you are ready to switch it to ${targetVersion}.`,
    );
  } else if (status.installed && !servesThisHome) {
    yield* Console.log(
      `  The background service serves ${status.installedBaseDir ?? "another Gentic2 home"} and was left unchanged.`,
    );
  }
