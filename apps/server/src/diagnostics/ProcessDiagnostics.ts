...
>()("g2/diagnostics/ProcessDiagnostics") {}
    ...
    function* (input) {
      if (input.pid === process.pid) {
        return {
          ...
          signal: input.signal,
          signaled: false,
          message: Option.some("Refusing to signal the G2 server process."),
        };
      }
      ...
      if (!canSignalCategory(selected.category)) {
        return {
          ...
          signal: input.signal,
          signaled: false,
          message: Option.some(`Process ${input.pid} is not a signalable G2 backend descendant.`),
        };
      }
