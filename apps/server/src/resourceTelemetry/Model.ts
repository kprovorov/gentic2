...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allG2: ResourceTelemetryAggregate;
  };
...
function applyLifecycleCounters(input: {
  ...
  let allG2 = input.counters.allG2;
  for (const processDelta of input.deltas) {
    ...
    allG2 = incrementCounters(allG2, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allG2 = incrementCounters(allG2, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allG2 = incrementCounters(allG2, { processExits: 1 });
  }

  return { backend, electron, monitor, allG2 };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allG2: aggregate(ordered, counters.allG2),
    },
    deltas: processDeltas,
  };
}
