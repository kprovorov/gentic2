...
import { VcsRepositoryDetectionError } from "@gentic2/contracts";
...
it.effect("routes GitHub remotes to the GitHub provider", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry({
      remotes: [{ name: "origin", url: "git@github.com:kprovorov/gentic2.git" }],
    });
...
it.effect("routes Bitbucket remotes to the Bitbucket provider", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry({
      remotes: [{ name: "origin", url: "git@bitbucket.org:kprovorov/gentic2.git" }],
    });
