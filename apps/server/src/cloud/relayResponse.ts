...
import { RelayProtectedError } from "@gentic2/contracts/relay";
...
export function relayRequestError(cause: unknown) {
  return isRelayResponseError(cause)
    ...
    : new EnvironmentHttpInternalServerError({
        message: `Could not complete the Gentic2 Connect relay request. ${isHttpClientError(cause) ? `The relay request failed (${cause.reason._tag}).` : "The relay returned an unexpected response."} Check this machine's network connection and relay availability, then retry.`,
      });
}
...
function recoveryHint(error: RelayProtectedError): string {
  switch (error._tag) {
    case "RelayEnvironmentLinkLimitExceededError":
      return "Unlink an unused environment in Gentic2 Connect, then restart Gentic2 on this machine.";
    case "RelayAuthInvalidError":
      return "Run `g2 connect login` to check this machine's authorization. If the stored credential was revoked, sign out with `g2 connect logout`, then run `g2 connect` again. Restart Gentic2 after signing in.";
    case "RelayEnvironmentLinkProofExpiredError":
    case "RelayEnvironmentLinkProofInvalidError":
      return "Check this machine's date and time, update Gentic2, then restart it.";
    default:
...
export const filterRelayResponse = Effect.fn("cloud.filter_relay_response")(function* (
  ...
  const message = Option.isSome(decoded)
    ? `Gentic2 Connect: ${decoded.value.message}. ${recoveryHint(decoded.value)} Trace ID: ${decoded.value.traceId}.`
    : `Gentic2 Connect relay returned HTTP ${response.status} without a recognized error response. Check relay access and any proxy or firewall restrictions, then restart Gentic2.${requestId}`;
