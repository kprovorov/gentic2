...
import { ServerSelfUpdateError, ThreadId } from "@gentic2/contracts";
    ...
    Effect.gen(function* () {
      if (request.url.endsWith("/SHA256SUMS")) {
        ...
        return HttpClientResponse.fromWeb(
          request,
          new Response(`${hex}  g2-1.1.0-linux-x64.tar.gz\n`),
        );
      }
      order.push("download");
...
const makeHarness = Effect.fn("test.make_self_update_harness")(function* (
  ...
  const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "g2-self-update-test-" });
  const order: string[] = [];
...
it.layer(NodeServices.layer)("server self update", (it) => {
  ...
  it.effect("rejects invalid versions and desktop-managed servers before staging", () =>
    Effect.gen(function* () {
      ...
      ).toBe("'latest' is not an exact g2 version.");
