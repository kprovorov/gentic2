...
import { isSecureRelayUrl } from "@gentic2/shared/relayUrl";
  ...
  Effect.mapError(
    () =>
      new EnvironmentHttpInternalServerError({
        message: "GENTIC2_RELAY_URL must be configured as a secure absolute HTTPS origin.",
      }),
  ),
);
...
const makeCloudLinkProof = Effect.fn("environment.cloud.makeLinkProof")(function* (
  ...
  const payload = {
    iss: `g2-env:${descriptor.environmentId}`,
...
const reconcileDesiredCloudLinkWith = Effect.fn("environment.cloud.reconcileDesiredLinkWith")(
  ...
  Effect.catchIf(
    ServerSecretStore.isSecretStoreError,
    failEnvironmentCloudInternalError("Could not persist desired Gentic2 Connect link state."),
  ),
  Effect.catchTags({
...
const makeManagedTunnelRecoveryProof = Effect.fn(
  ...
  const claims = {
    iss: `g2-env:${input.environmentId}`,
...
export const recoverManagedCloudTunnel = Effect.fn("environment.cloud.recoverManagedCloudTunnel")(
  function* (
    ...
    if (recovered.endpointRuntime.providerKind !== "cloudflare_tunnel") {
      return yield* new EnvironmentHttpInternalServerError({
        message: "Gentic2 Connect returned an unsupported managed tunnel configuration.",
      });
    }
...
// explicit launcher stop (`g2 service uninstall`, `systemctl stop`,
...
export const releaseManagedTunnelOnShutdown = Effect.fn(
  ...
  // persisted URL: GENTIC2_RELAY_URL may have changed since the link was made.
...
export const connectHttpApiLayer = HttpApiBuilder.group(
  ...
  Effect.fnUntraced(function* (handlers) {
    ...
    return handlers
      ...
      .handle("g2MintCredential", ({ payload }) =>
