...
const linuxRuntime = "/home/theo/.g2/runtime/versions/1.2.3/g2";
const linuxPlan = {
  program: [linuxRuntime, "__service-launcher"],
  baseDir: "/home/theo/.g2",
  logPath: "/home/theo/.g2/userdata/logs/boot-service.log",
  unitPath: "/home/theo/.config/systemd/user/gentic2.service",
};
...
it("reads the served Gentic2 home back out of a rendered unit or plist", () => {
  const plan = (baseDir: string) => ({
    program: [`${baseDir}/runtime/versions/1.2.3/g2`, "__service-launcher"],
    baseDir,
  ...
  expect(
    BootService.bootServiceBaseDirOf(BootService.renderBootServiceUnit(plan("/home/theo/.g2"))),
  ).toBe("/home/theo/.g2");
  // Spaces and specifiers are quoted and escaped on the way in.
  expect(
    BootService.bootServiceBaseDirOf(
      BootService.renderBootServiceUnit(plan("/home/theo/G2 Data/100%")),
    ),
  ).toBe("/home/theo/G2 Data/100%");
  expect(
    BootService.bootServiceBaseDirOf(
...
const macRuntime = "/Users/theo/.g2/runtime/versions/1.2.3/g2";
const macPlan = {
  program: [macRuntime, "__service-launcher"],
  baseDir: "/Users/theo/.g2",
  logPath: "/Users/theo/.g2/userdata/logs/boot-service.log",
  unitPath: "/Users/theo/Library/LaunchAgents/com.gentic2.gentic2.service.plist",
};
...
it("appends both stdio streams to the boot service log", () => {
  ...
  expect(plist).toContain(
    "<key>StandardOutPath</key>\n  <string>/Users/theo/.g2/userdata/logs/boot-service.log</string>",
  );
  expect(plist).toContain(
...
it("escapes XML in host paths", () => {
  const plist = BootService.renderBootServicePlist(
    { ...macPlan, baseDir: "/Users/theo/G2 & <Co>" },
    { homeDir: "/Users/theo", environmentPath: "/Users/theo/Tools & <Scripts>:/usr/bin" },
  );

  expect(plist).toContain("<string>/Users/theo/G2 &amp; &lt;Co&gt;</string>");
...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  const home = yield* fs.makeTempDirectoryScoped({ prefix: "g2-boot-service-test-" });
  const baseDir = path.join(home, ".g2");
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable gentic2.service") control.enabled = true;
      if (!failed && command === "systemctl --user restart gentic2.service") control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop gentic2.service" ||
        ...
        stdout:
          input.args[0] === "--version"
            ? // The runtime under test reports the version of the directory it
              ...
              `g2 v${/versions\/([^/]+)\//.exec(input.command)?.[1] ?? "1.2.3"}\n`
    ...
    Effect.gen(function* () {
      ...
      Effect.provide(
        Layer.mergeAll(
          ...
          Layer.succeed(HostProcessExecutablePath, "/usr/bin/g2"),
          Layer.succeed(
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop gentic2.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now gentic2.service")).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? ["systemctl --user stop gentic2.service", "systemctl --user restart gentic2.service"]
            : [
                "launchctl bootout --wait gui/501/com.gentic2.gentic2.service",
  ...
  it.effect("install with start=false rewrites the files and marks a restart pending", () =>
    Effect.gen(function* () {
      ...
      expect(yield* fs.readFileString(plan.unitPath)).toContain("versions/1.2.4/g2");
      expect(
        commands.filter(
      ...
      // the reason is named so `g2 service status` can point at restart.
  ...
  it.effect("restart stops and starts an installed service, and is a no-op otherwise", () =>
    Effect.gen(function* () {
      ...
      expect(
        ...
        "systemctl --user stop gentic2.service",
        "systemctl --user daemon-reload",
        "systemctl --user enable gentic2.service",
        "systemctl --user restart gentic2.service",
      ]);
    }),
  );

  it.effect("restart leaves a service that serves another Gentic2 home alone", () =>
    Effect.gen(function* () {
      const { service, fs, commands, makeService } = yield* makeHarness();
      yield* service.install();
      commands.length = 0;
      const path = yield* Path.Path;
      const otherHome = yield* fs.makeTempDirectoryScoped({ prefix: "g2-other-home-" });

      const other = yield* makeService(undefined, "1.2.3", path.join(otherHome, ".g2"));
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "com.gentic2.gentic2.service.plist"),
        ),
      ).toBe(true);
      expect(yield* fs.readFileString(plan.unitPath)).toContain(
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/com.gentic2.gentic2.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.gentic2.gentic2.service",
        "launchctl enable gui/501/com.gentic2.gentic2.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/com.gentic2.gentic2.service";
