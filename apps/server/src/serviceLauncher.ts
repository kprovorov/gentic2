...
const runtimePaths = (baseDir: string, version: string) => {
  ...
  // oxlint-disable-next-line gentic2/no-global-process-runtime -- Standalone launcher has no Effect runtime.
  const executableName = process.platform === "win32" ? "g2.exe" : "g2";
  return {
    versionDir,
...
export class Launcher {
  ...
  async #recover(): Promise<void> {
    ...
    // handoff release its tunnel. A restart deferred by `g2 update` is done
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected g2@${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
...
export async function main(): Promise<void> {
  const baseDir = process.env.GENTIC2_HOME?.trim();
  if (baseDir === undefined || baseDir === "") {
    throw new Error("GENTIC2_HOME is required by the Gentic2 service launcher.");
  }
