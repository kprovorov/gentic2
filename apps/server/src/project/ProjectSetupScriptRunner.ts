...
>()("g2/project/ProjectSetupScriptRunner") {}
 ...
 */
const COMPLETION_SENTINEL_PREFIX = "__G2_SETUP_DONE__";
...
function wrapCommandForCompletion(
  ...
  switch (shell) {
    case "powershell":
      return `$global:LASTEXITCODE = $null; & {\r${body}\r}; if ($null -ne $LASTEXITCODE) { $__g2c = $LASTEXITCODE } elseif ($?) { $__g2c = 0 } else { $__g2c = 1 }; Write-Host "${sentinel}$__g2c"`;
    case "fish":
