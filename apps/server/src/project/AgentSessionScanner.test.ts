    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        ServerConfig.layerTest(
          ...
          input.configBaseDir ?? { prefix: "gentic2-scanner-config-" },
        ),
...
it.layer(NodeServices.layer)("AgentSessionScanner", (it) => {
  describe("scan", () => {
    it.effect("reads Claude project cwds from transcripts, newest first", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("gentic2-claude-home-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        const olderWorkspace = yield* makeTempDir("gentic2-workspace-older-");
        const newerWorkspace = yield* makeTempDir("gentic2-workspace-newer-");
    ...
    it.effect("groups Codex rollouts by cwd across date directories", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("gentic2-claude-home-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        const workspace = yield* makeTempDir("gentic2-workspace-");
        const otherWorkspace = yield* makeTempDir("gentic2-workspace-other-");
    ...
    it.effect("returns the imported project ID through a realpath alias", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        const workspace = yield* makeTempDir("gentic2-workspace-");
        const linkParent = yield* makeTempDir("gentic2-scanner-links-");
    ...
    it.effect("keeps case variants distinct when the filesystem identities differ", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("gentic2-claude-home-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        const backingUpper = yield* makeTempDir("gentic2-backing-upper-");
        const backingLower = yield* makeTempDir("gentic2-backing-lower-");
        const aliasParent = yield* makeTempDir("gentic2-case-aliases-");
    ...
    it.effect("uses explicit provider instance homes instead of overridden legacy homes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("gentic2-claude-legacy-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-legacy-");
        const claudeInstanceHome = yield* makeTempDir("gentic2-claude-instance-");
        const codexInstanceHome = yield* makeTempDir("gentic2-codex-instance-");
        const legacyWorkspace = yield* makeTempDir("gentic2-workspace-legacy-");
        const claudeWorkspace = yield* makeTempDir("gentic2-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("gentic2-workspace-codex-");
    ...
    it.effect("scans each distinct home across multiple instances once", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("gentic2-claude-home-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        const otherCodexHome = yield* makeTempDir("gentic2-codex-other-");
        const workspace = yield* makeTempDir("gentic2-workspace-");
        const otherWorkspace = yield* makeTempDir("gentic2-workspace-other-");
    ...
    it.effect("honors provider instance home directory environment variables", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("gentic2-claude-legacy-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-legacy-");
        const claudeEnvironmentHome = yield* makeTempDir("gentic2-claude-env-");
        const codexEnvironmentHome = yield* makeTempDir("gentic2-codex-env-");
        const claudeWorkspace = yield* makeTempDir("gentic2-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("gentic2-workspace-codex-");
    ...
    it.effect("does not scan provider instances disabled by the envelope or config", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("gentic2-claude-home-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        const envelopeDisabledHome = yield* makeTempDir("gentic2-codex-disabled-envelope-");
        const configDisabledHome = yield* makeTempDir("gentic2-codex-disabled-config-");
        const envelopeWorkspace = yield* makeTempDir("gentic2-workspace-disabled-envelope-");
        const configWorkspace = yield* makeTempDir("gentic2-workspace-disabled-config-");
    ...
    it.effect("excludes the home directory, temporary root, and G2 data directory", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("gentic2-claude-home-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        const configBaseDir = yield* makeTempDir("gentic2-scanner-base-");
        const workspace = yield* makeTempDir("gentic2-workspace-");
    ...
    it.effect("excludes G2-managed worktree sandboxes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("gentic2-claude-home-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        ...
        const worktreeCwd = path.join(claudeHomePath, ".g2", "worktrees", "gentic2", "wt-1");
    ...
    it.effect("excludes Codex scratch directories and Downloads", () =>
      Effect.gen(function* () {
        ...
        const runId = path.basename(yield* makeTempDir("gentic2-scanner-test-"));
        ...
        const keep = yield* makeTempDir("gentic2-workspace-keep-");
    ...
    it.effect("skips linked git worktrees and reports the origin of real checkouts", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("gentic2-claude-home-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        const repo = yield* makeTempDir("gentic2-workspace-repo-");
        const worktree = yield* makeTempDir("gentic2-workspace-worktree-");
        const plain = yield* makeTempDir("gentic2-workspace-plain-");
        const noRemote = yield* makeTempDir("gentic2-workspace-noremote-");
        const submodule = yield* makeTempDir("gentic2-workspace-submodule-");
        ...
        expect(
          ...
          {
            path: repo,
            git: { remoteKey: "github.com/kprovorov/gentic2", repository: "kprovorov/gentic2" },
          },
        ]);
      }),
    );

    it.effect("excludes sandboxes under the configured worktrees dir without .g2 in the path", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("gentic2-claude-home-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        ...
        // dir contains no `.g2` segment — only the config-based prefix match
        // can exclude this one.
        const worktreeCwd = path.join(configBaseDir, "worktrees", "gentic2", "wt-2");
        yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
        yield* writeTranscript({
          filePath: path.join(claudeHomePath, "projects", "-slug", "a.jsonl"),
          contents: claudeSessionLine(worktreeCwd),
    ...
    it.effect("excludes sandboxes reached through a symlink into the worktrees dir", () =>
      Effect.gen(function* () {
        ...
        const worktreeCwd = path.join(configBaseDir, "worktrees", "gentic2", "wt-3");
        yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
    ...
    it.effect.each([64, 65])("shares metadata bytes across homes for %s one-MiB files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("gentic2-metadata-home-");
        const secondHome = yield* makeTempDir("gentic2-metadata-second-");
        const codexHomePath = yield* makeTempDir("gentic2-metadata-codex-");
        const firstWorkspace = yield* makeTempDir("gentic2-metadata-first-project-");
        const secondWorkspace = yield* makeTempDir("gentic2-metadata-second-project-");
        const directories = [
    ...
    it.effect.each([50, 51])("bounds metadata open/read calls for %s short-read files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("gentic2-short-metadata-home-");
        const codexHomePath = yield* makeTempDir("gentic2-short-metadata-codex-");
        const workspace = yield* makeTempDir("gentic2-short-metadata-project-");
    ...
    it.effect("bounds malformed metadata records without excluding another account", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("gentic2-record-metadata-home-");
        const secondHome = yield* makeTempDir("gentic2-record-metadata-second-");
        const codexHomePath = yield* makeTempDir("gentic2-record-metadata-codex-");
        const workspace = yield* makeTempDir("gentic2-record-metadata-project-");
        const directory = path.join(claudeHomePath, "projects", "p");
        const template = path.join(directory, "template.jsonl");
        yield* writeTranscript({
          filePath: template,
    ...
    it.effect.each([19_999, 20_000])(
      ...
      (count) =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const fileSystem = yield* FileSystem.FileSystem;
          const claudeHomePath = yield* makeTempDir("gentic2-directory-budget-home-");
          const codexHomePath = yield* makeTempDir("gentic2-directory-budget-codex-");
    ...
    it.effect("returns an empty result when neither home directory exists", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const root = yield* makeTempDir("gentic2-missing-homes-");
  ...
  describe("recentThreads", () => {
    it.effect.each([false, true])(
      ...
      (overflow) =>
        Effect.gen(function* () {
          ...
          const claudeHomePath = yield* makeTempDir("gentic2-record-limit-claude-");
          const codexHomePath = yield* makeTempDir("gentic2-record-limit-codex-");
          const workspace = yield* makeTempDir("gentic2-record-limit-project-");
    ...
    it.effect("keeps the provider instance that owns a custom session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("gentic2-claude-home-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        const customHome = yield* makeTempDir("gentic2-codex-custom-");
        const workspace = yield* makeTempDir("gentic2-workspace-");
    ...
    it.effect("streams large transcripts across providers without hiding projects", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("gentic2-budget-claude-");
        const codexHomePath = yield* makeTempDir("gentic2-budget-codex-");
        const workspace = yield* makeTempDir("gentic2-budget-workspace-");
    ...
    it.effect("skips excessive records without blocking an older valid transcript", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("gentic2-record-budget-claude-");
        const codexHomePath = yield* makeTempDir("gentic2-record-budget-codex-");
        const workspace = yield* makeTempDir("gentic2-record-budget-workspace-");
        ...
        it.effect.skipIf(replacement === "symlink alias" && !symlinksSupported)(
          ...
          () =>
            Effect.gen(function* () {
              ...
              const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
              yield* TestClock.setTime(nowMs);
              const fixture = yield* makeTempDir("gentic2-replaced-cwd-");
    ...
    it.effect("checks file identity and provider before skipping completed history", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("gentic2-completed-claude-");
        const codexHomePath = yield* makeTempDir("gentic2-completed-codex-");
        const workspace = yield* makeTempDir("gentic2-completed-workspace-");
    ...
    it.effect("does not reopen a transcript that becomes a non-file after discovery", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        const workspace = yield* makeTempDir("gentic2-workspace-");
        const nonFilePath = yield* makeTempDir("gentic2-non-file-");
    ...
    it.effect("does not import sessions from a G2-managed worktree", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        ...
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        const configBaseDir = yield* makeTempDir("gentic2-scanner-base-");
        const workspace = path.join(configBaseDir, "worktrees", "gentic2", "managed-worktree");
    ...
    it.effect("uses one deterministic provider instance for a shared session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("gentic2-claude-home-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        const sharedHome = yield* makeTempDir("gentic2-codex-shared-");
        const workspace = yield* makeTempDir("gentic2-workspace-");
    ...
    it.effect("keeps a second account when the first has 5000 newer files", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("gentic2-claude-home-");
        const codexHomePath = yield* makeTempDir("gentic2-codex-home-");
        const oldWorkspace = yield* makeTempDir("gentic2-workspace-old-");
        const recentWorkspace = yield* makeTempDir("gentic2-workspace-recent-");
        const recentHome = yield* makeTempDir("gentic2-claude-recent-home-");
