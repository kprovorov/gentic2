...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(G2ProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "g2.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("G2ProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid g2.json", () =>
      Effect.gen(function* () {
        const loader = yield* G2ProjectFileLoader.G2ProjectFileLoader;
    ...
    it.effect("returns none when g2.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* G2ProjectFileLoader.G2ProjectFileLoader;
        const cwd = yield* makeTempDir;
