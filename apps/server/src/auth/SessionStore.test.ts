...
const makeServerConfigLayer = (overrides?: Partial<ServerConfig.ServerConfig["Service"]>) =>
  ...
  ).pipe(Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "g2-auth-session-test-" })));
...
it.layer(NodeServices.layer)("SessionStore.layer", (it) => {
  it.effect("keys remote cookies by environment identity instead of state directory", () =>
    Effect.gen(function* () {
      ...
      const original = yield* cookieName("/srv/g2-one", EnvironmentId.make("environment-one"));
      const moved = yield* cookieName("/srv/g2-moved", EnvironmentId.make("environment-one"));
      const other = yield* cookieName("/srv/g2-one", EnvironmentId.make("environment-two"));
  ...
  it.effect("keeps reusable dev auth local across disk-backed stores and restarts", () =>
    Effect.gen(function* () {
      ...
      const baseA = yield* fs.makeTempDirectoryScoped({ prefix: "g2-dev-auth-a-" });
      const baseB = yield* fs.makeTempDirectoryScoped({ prefix: "g2-dev-auth-b-" });
  ...
  it.effect("invalidates old dev credentials and tickets after rotation or removal", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "g2-dev-auth-rotation-" });
