...
import { EnvironmentHttpApi } from "@gentic2/contracts";
...
).pipe(Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "g2-auth-http-test-" })));
    ...
    return yield* Effect.acquireUseRelease(
      ...
      ([environmentA, environmentB]) =>
        Effect.tryPromise(async () => {
          ...
          const devCookie = devCookies.find((cookie) => cookie.startsWith("g2_dev_session_"));
          expect(devCookie).toContain("HttpOnly");
          expect(devCookie).toContain(`=${DEV_TOKEN};`);
          expect(devCookies).toContainEqual(
            expect.stringMatching(/^g2_session_[^=]*=;.*Max-Age=0/),
          );
          ...
          expect(restrictedCookies[0]).toMatch(/^g2_session_/);
          expect(restrictedCookies[0]).not.toContain("g2_dev_session_");
        }),
      ([environmentA, environmentB]) =>
