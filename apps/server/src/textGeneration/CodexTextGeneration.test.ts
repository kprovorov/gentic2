...
import { CodexSettings, ProviderInstanceId, TextGenerationError } from "@gentic2/contracts";
...
const CodexTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "gentic2-codex-text-generation-test-",
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    ...
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "gentic2-codex-text-" });
...
it.layer(CodexTextGenerationTestLayer)("CodexTextGeneration", (it) => {
  ...
  it.effect("uses GENTIC2_CODEX_LAUNCH_ARGS for codex exec over settings", () =>
    withFakeCodexEnv(
      {
        output: JSON.stringify({
          subject: "Add important change",
        ...
        environment: {
          ...process.env,
          GENTIC2_CODEX_LAUNCH_ARGS: " --strict-config --listen off ",
        },
        requireArg: "--strict-config",
