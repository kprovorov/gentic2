...
> =>
  Effect.flatMap(Effect.service(FileSystem.FileSystem), (fs) =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "gentic2-terminal-" });
...
it.layer(
  ...
  it.effect("attaches to exited sessions without restarting them", () =>
    Effect.gen(function* () {
      ...
      const unsubscribe = yield* manager.attachStream(
        openInput({
          env: {
            GENTIC2_WORKTREE_PATH: "/tmp/should-not-restart",
          },
      ...
      const unsubscribe = yield* manager.attachStream(
        {
          ...openInput({
            env: {
              GENTIC2_WORKTREE_PATH: "/tmp/restart-requested",
            },
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      expect(spawnInput.env.GENTIC2_PORT).toBeUndefined();
  ...
  it.effect("strips AppImage runtime env from terminal sessions", () =>
    Effect.gen(function* () {
      const appDir = "/tmp/.mount_Gentic2abc123";
      const { manager, ptyAdapter } = yield* createManager(5, {
        env: {
          APPIMAGE: "/home/user/Gentic2.AppImage",
          APPDIR: appDir,
          ARGV0: "/home/user/Gentic2.AppImage",
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            GENTIC2_PROJECT_ROOT: "/repo",
            GENTIC2_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
            NO_COLOR: "1",
            FORCE_COLOR: "0",
          },
      ...
      assert.equal(spawnInput.env.GENTIC2_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.GENTIC2_WORKTREE_PATH, "/repo/worktree-a");
  ...
  it.effect("resolves a provider instance environment before spawning", () =>
    Effect.gen(function* () {
      ...
      const { manager, ptyAdapter } = yield* createManager(5, {
        env: { GENTIC2_SECRET: "server-only" },
      ...
      expect(ptyAdapter.spawnInputs[0]?.env.GENTIC2_SECRET).toBeUndefined();
    ...
    Effect.gen(function* () {
      ...
      Effect.provide(
        ServerSettings.layer.pipe(
          ...
          Layer.provide(
            ServerConfig.layerTest(process.cwd(), { prefix: "gentic2-terminal-provider-restart-" }),
          ),
        ),
      ),
    ),
