...
it.effect("launches an installed editor with platform-safe arguments", () =>
  Effect.gen(function* () {
    ...
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "g2-editors-" });
...
// oxlint-disable-next-line gentic2/no-global-process-runtime -- the skip decision needs the real host platform, outside any Effect runtime.
it.skipIf(process.platform !== "win32")(
  "delivers the raw /select switch for spaced paths through real PowerShell",
  { timeout: 60_000 },
  async () => {
    const tempDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "g2-reveal-smoke-"));
    try {
  ...
  () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.gen(function* () {
        ...
        yield* launcher.launchEditor({
          editor: "file-manager",
          cwd: "/home/g2/workspace/media/clip.mp4",
          reveal: true,
        });
        return { kind, editors };
      }).pipe(
      ...
      assert.equal(
        decodedCommand,
        "$ProgressPreference = 'SilentlyContinue'; Start-Process 'explorer.exe' -ArgumentList ('/select,\"' + '\\\\wsl.localhost\\Ubuntu-24.04\\home\\g2\\workspace\\media\\clip.mp4' + '\"')",
      );
    }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
);
...
it.effect.skipIf(windowsHost)(
  ...
  () =>
    Effect.gen(function* () {
      ...
      assert.ok(launch);
      assert.deepEqual(launch.args, ["/home/g2/workspace/media"]);
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        yield* launcher.launchEditor({
          editor: "file-manager",
          cwd: '/home/g2/work "quoted"/clip.mp4',
          reveal: true,
        });
      }).pipe(
        Effect.provide(
      ...
      assert.deepEqual(spawned.args, ['\\\\wsl.localhost\\Ubuntu-24.04\\home\\g2\\work "quoted"']);
    }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
);
  ...
  it.effect.skipIf(windowsHost && platform !== "win32")(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const home = yield* fs.makeTempDirectoryScoped({ prefix: "g2-editor installs-" });
...
it.effect.skipIf(windowsHost)("ignores unusable app bundles and keeps PATH launchers first", () =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const home = yield* fs.makeTempDirectoryScoped({ prefix: "g2-editor-priority-" });
      ...
      Layer.mergeAll(
        ...
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: {
              PATH: "C:\\g2-editor-discovery-cache-test",
              PATHEXT: ".COM;.EXE;.BAT;.CMD",
            },
          }),
        ),
    ...
    Effect.provide(
      Layer.mergeAll(
        ...
        ConfigProvider.layer(
          ConfigProvider.fromEnv({
            env: {
              PATH: "C:\\g2-editor-discovery-interrupt-test",
              PATHEXT: ".COM;.EXE;.BAT;.CMD",
            },
          }),
        ),
