...
import type { AntigravityAuthMethod, ProviderInstanceId } from "@gentic2/contracts";
...
export const ANTIGRAVITY_AUTH_BROWSER_MARKER = "__G2_ANTIGRAVITY_AUTH_URL__";
...
const browserHelperSource =
  ...
  `()=>process.exit(0))`;
const browserPreflightUrl = "https://example.invalid/g2-antigravity-browser-preflight";
...
/** Recognizes native auth failures and interactive login blocked by G2. */
...
function antigravityEnvironment(
  ...
  // Point it at a G2-owned directory so the driver can reclaim the space.
...
/**
 * The agent reads its user-global skills under `GEMINI_HOME`, which G2 points
...
export const prepareAntigravityProfile = Effect.fn("prepareAntigravityProfile")(function* (input: {
  ...
  if (
    ...
    return yield* authSupportError(
      "The G2 runtime path cannot be used to suppress Antigravity browser launches.",
    );
  }
...
/** Receives native 1.1.1 sign-in URLs and G2 browser-helper URLs without logging stderr. */
