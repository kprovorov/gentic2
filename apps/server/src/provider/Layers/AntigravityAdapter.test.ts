...
const layer = ServerConfig.layerTest(process.cwd(), {
  prefix: "g2-antigravity-adapter-test-",
...
it.layer(layer)("AntigravityAdapter", (it) => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "g2-antigravity-transport-",
        });
          ...
          makeRuntime: (input) =>
            makeAntigravityAcpRuntime({
              ...
              spawn: {
                ...
                env: {
                  ...process.env,
                  G2_ACP_ANTIGRAVITY: "1",
                  G2_ACP_REQUEST_LOG_PATH: requestLog,
                },
                extendEnv: false,
              },
  ...
  it.effect("serves client file reads and writes only inside the session roots", () =>
    Effect.gen(function* () {
      ...
      const cwd = yield* fs.makeTempDirectoryScoped({ prefix: "g2-agy-fs-" });
      const outside = yield* fs.makeTempDirectoryScoped({ prefix: "g2-agy-outside-" });
