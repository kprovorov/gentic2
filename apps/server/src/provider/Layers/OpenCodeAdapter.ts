...
export function makeOpenCodeAdapter(
  ...
  return Effect.gen(function* () {
    ...
    // G2 supplies the message ID to match prompt admission events. Keep OpenCode's sortable native shape so equal-time messages retain their upstream order.
    ...
    const failPromptAdmissionRecovery = Effect.fn("failPromptAdmissionRecovery")(function* (
      ...
      const detail =
        "OpenCode accepted the prompt, but Gentic2 could not confirm its message or session status.";
    ...
    const rollbackThread: OpenCodeAdapterShape["rollbackThread"] = Effect.fn("rollbackThread")(
      function* (threadId, numTurns) {
        ...
        if (target) {
          ...
          // conversation so G2 alone decides whether filesystem changes survive.
