...
const makeHarness = Effect.fn("makeAntigravityDriverHarness")(function* (
  ...
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "g2-antigravity-driver-" });
  ...
  const instance = yield* AntigravityDriver.create({
    ...
    environment: [
      ...
      { name: "G2_ACP_ANTIGRAVITY", value: "1" },
      { name: "G2_ACP_REQUEST_LOG_PATH", value: requestLog },
...
const testLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "g2-antigravity-driver-config-",
}).pipe(
