...
const testLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "g2-cursor-driver-copy-command-",
}).pipe(
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "g2-cursor-driver-" });
    ...
    Effect.gen(function* () {
      const instance = yield* CursorDriver.create({
        ...
        config: {
          ...
          binaryPath: NodePath.join(NodeOS.tmpdir(), "g2-cursor-missing", "cursor-agent"),
        },
      });
