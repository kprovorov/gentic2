...
import type { ProviderUserInputAnswers, UserInputQuestion } from "@gentic2/contracts";
...
/**
 * Client captured the plan for G2's proposed-plan card. Abandon the native
 * Grok plan-approval gate so the turn unblocks; the user implements via G2 UI.
 */
 ...
 * Used so G2 can show the plan while plan mode is still active (before exit).
 */
...
export const makeXAiPromptCompletionRuntime = Effect.fn("makeXAiPromptCompletionRuntime")(
  function* (runtime: AcpSessionRuntime.AcpSessionRuntime["Service"]) {
    ...
    const allocatePromptFallbackId = Effect.sync(() => {
      ...
      return `g2-xai-prompt-${nextPromptFallbackId}`;
    });
