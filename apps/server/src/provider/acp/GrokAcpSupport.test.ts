...
describe("grokAcpSpawnArgs", () => {
  it("inherits the Grok CLI config when no G2 runtime mode is set", () => {
    expect(grokAcpSpawnArgs()).toEqual(["agent", "stdio"]);
  });

  it("forces Grok to ask when G2 is Supervised", () => {
...
describe("buildGrokAcpSpawnInput", () => {
  it("passes the Gentic2 referrer through Grok OAuth env", () => {
