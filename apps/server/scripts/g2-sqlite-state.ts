...
export class SqliteStateDatabaseMissingError extends Schema.TaggedError<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start G2 once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedError<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.g2 database. Use an isolated --base-dir.";
  }
}
...
const g2SqliteStateCommand = Command.make(
  "g2-sqlite-state",
  {
    ...
    baseDir: Flag.String("base-dir").pipe(
      Flag.withDescription("Explicit G2 base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated G2 SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(g2SqliteStateCommand, { version: "0.0.0" }).pipe(
