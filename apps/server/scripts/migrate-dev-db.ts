 ...
 * ~/.g2 database, then run this checkout's migrations against it.
 *
 * `vp run migrate-dev-db` from a worktree:
 *   1. Nukes `<worktree>/.g2/userdata/state.sqlite`.
...
import * as NodeSqliteClient from "@gentic2/shared/nodeSqliteClient";
...
export class MigrateDevDbNotInWorktreeError extends Schema.TaggedError<MigrateDevDbNotInWorktreeError>()(
  ...
  override get message(): string {
    return "Not inside a linked git worktree. Pass --base-dir to target an isolated .g2 directory.";
  }
}
...
export class MigrateDevDbSharedHomeError extends Schema.TaggedError<MigrateDevDbSharedHomeError>()(
  ...
  override get message(): string {
    return "Refusing to rebuild the shared ~/.g2 database. Use an isolated --base-dir.";
  }
}
...
export class MigrateDevDbServerRunningError extends Schema.TaggedError<MigrateDevDbServerRunningError>()(
  ...
  override get message(): string {
    return `Dev database at '${this.databasePath}' is open by a running server (pid ${this.pid} per server-runtime.json). Stop that server first; if that pid is not actually a G2 server (stale descriptor, reused pid), delete the server-runtime.json next to the database and retry.`;
  }
}
...
export interface RunMigrateDevDbInput {
  /** Isolated .g2 directory. Defaults to `<worktree>/.g2` of the cwd. */
  readonly baseDir?: string | undefined;
  /** Source database. Defaults to `~/.g2/userdata/state.sqlite`. */
...
export const runMigrateDevDb = Effect.fn("runMigrateDevDb")(function* (
  ...
  const sharedHome = path.resolve(options.sharedHome ?? path.join(NodeOS.homedir(), ".g2"));
  ...
  const baseDir =
    input.baseDir !== undefined
      ...
      : yield* resolveWorktreeG2Home(process.cwd());
...
export const migrateDevDbCommand = Command.make(
  "migrate-dev-db",
  {
    ...
    baseDir: Flag.String("base-dir").pipe(
      Flag.optional,
      Flag.withDescription("Isolated .g2 directory. Defaults to the current worktree's .g2."),
    ),
    source: Flag.String("source").pipe(
      Flag.optional,
      Flag.withDescription("Source database. Defaults to ~/.g2/userdata/state.sqlite."),
    ),
  },
  ...
  Command.withDescription(
    "Rebuild the worktree dev database from a pruned snapshot of the real ~/.g2 data, then run migrations.",
  ),
);
