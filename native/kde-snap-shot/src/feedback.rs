...
const OBJECT: &str = "/com/gentic2/KdeCapture/Feedback";
...
#[zbus::interface(name = "com.gentic2.KdeCapture.Feedback")]
impl Bridge {
...
pub(super) fn run(connection: &Connection, directory: &Path, options: &str) -> Result<()> {
    ...
    let name = format!("g2-capture-feedback-{}", std::process::id());
...
mod tests {
    ...
    fn only_kwin_can_read_commands_or_acknowledge_feedback() {
        ...
        let unrelated = Proxy::new(
            ...
            destination.as_str(),
            OBJECT,
            "com.gentic2.KdeCapture.Feedback",
        )
        .unwrap();
