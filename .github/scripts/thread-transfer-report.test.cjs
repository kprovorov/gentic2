...
test("renders baseline, impact, ceiling, and ceiling changes", () => {
  ...
  const comment = renderComment({
    ...
    currentRun: {
      ...
      conclusion: "success",
      url: "https://github.com/kprovorov/gentic2/actions/runs/2",
    },
    baselineRun: {
      sha: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
      matchesBase: true,
      url: "https://github.com/kprovorov/gentic2/actions/runs/1",
    },
  });
  ...
  assert.match(comment, /<!-- g2-thread-transfer-report -->/);
  assert.match(
    comment,
    /<!-- g2-thread-transfer-result-sha:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb -->/,
  );
});
...
test("resolves a fallback PR with a redacted head repo and exact main baseline", async () => {
  ...
  await resolve({
    github,
    context: {
      repo: { owner: "pingdotgg", repo: "gentic2" },
      payload: {
        workflow_run: {
          id: 2,
          event: "pull_request",
          ...
          head_repository: { full_name: "kprovorov/gentic2" },
          conclusion: "success",
          pull_requests: [],
        },
      },
      ...
      paginate: async (method) => {
        ...
        return [5350, 5351].map((number) => ({
          ...
          head: {
            sha: "head-sha",
            ref: "feature-branch",
            repo: { full_name: "kprovorov/gentic2" },
          },
        }));
      },
      rest: {
      ...
      paginate: async () => [
        {
          ...
          body: `<!-- g2-thread-transfer-report -->\n<!-- g2-thread-transfer-result-sha:${sha} -->`,
        },
      ],
      rest: {
        issues: {
