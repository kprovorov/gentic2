...
`<app-id>.SnapShot` on the session bus and exports `com.gentic2.SnapShot.Capture`; the config
...
Source in `apps/desktop/gnome-extension`, UUID `snap-shot@gentic2.com`. GNOME only discovers a newly
...
The extension trusts callers that own `com.gentic2.Gentic2.SnapShot` (or the `.Development`
...
inside the extension with coordinates relative to G2's content area. Electron 44's restored-session
