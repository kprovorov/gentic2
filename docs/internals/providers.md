...
G2-managed OpenCode chat uses one server per thread. Its MCP registrations are directory-scoped, while
G2's MCP connection is thread-scoped. Sharing a chat server between threads in one directory would
...
G2 auth session. The client carries the return URL back to the environment because the provider's
