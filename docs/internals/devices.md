...
Device panel work over Tailscale and Gentic2 Connect, including when an SSH host runs the devices.
...
npm-installed at a pinned version into the Gentic2 home after its matching Device
...
session arbitrary command execution on the host, so G2 does not. The
...
snapshot model agents need and stays current with its own releases. G2 prepends
