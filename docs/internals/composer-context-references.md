...
```text
[label](g2-context://v1/<kind>/<contextId>)
![label](g2-context://v1/image/<contextId>)
```

The parser accepts exactly the `g2-context:` scheme, the `v1` host, one kind segment matching
...
2. A trailing `<g2_context version="1">` envelope holds one `<context kind id>` entry per unique
...
3. Captured text is data: any `<` that would open or close `g2_context` or `context` is escaped,
...
is, older messages are upgraded in memory. `ChatMarkdown` renders `g2-context://` links through
...
chips, a structured fragment under `web application/x-g2-context-fragment+json`
