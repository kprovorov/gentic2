...
Shift keys together. Gentic2 attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while Gentic2 is in front captures Gentic2 itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. Gentic2 refuses shortcuts
...
terminal contents. If an app is slow to answer, Gentic2 attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **Gentic2 SnapShots** extension during setup. It is installed
...
shortcut does not fire, check Gentic2 under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. Gentic2 shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. Gentic2 shows the exact binding it will
...
**Other Wayland desktops.** Gentic2 uses the desktop's screenshot portal when it can capture the
