...
Gentic2 requires OpenCode 1.14.19 or newer, including when you connect an existing
OpenCode server.

## Local or external server

Leave **Server URL** empty to let Gentic2 start OpenCode locally. A password in
provider settings applies to both that server and Gentic2's connection. With no
...
settings. Gentic2 uses only that configured password for an external server; it
...
restart before Gentic2 can see configuration changes.
