...
Gentic2 uses Claude Code's login and configuration. Start with the default provider
...
Gentic2 uses the Claude configuration on the connected server.
...
Select the model you want in Gentic2. For an OpenRouter model outside the built-in
...
do not replace the explicit model ID selected in Gentic2. Custom models may have
