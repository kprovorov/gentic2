...
Gentic2 runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for Gentic2 to confirm
...
hostname. Only that Gentic2 sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. Gentic2's separate Plan mode
...
Gentic2 keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. Gentic2 also reads `.gemini/skills` and the
