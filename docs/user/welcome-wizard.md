...
Gentic2 shows a setup flow when you open a new installation or connect to the
hosted app for the first time. Existing workspaces skip this flow.

## Connect your computers

Select one or more computers to set up. If you opened Gentic2 directly from a
...
- **Gentic2 Connect** connects computers that are signed in to your account.
  [Install the CLI](./install.md#command-line) and run `g2 connect` on each
  computer you want to add, then start Gentic2 or run `g2 serve` so the
  computer stays available.
- **Add a computer** connects directly to a server on your network or tailnet.
  Start the server with `g2 serve`, then run `g2 pair --tailscale` and paste
  the pairing link. You can also run `g2 serve --host <address>` and use
  `g2 pair` when the server is already reachable on your network.

Saved computers and computers discovered through Gentic2 Connect are selected by
...
If Gentic2 cannot confirm the workspace during startup, the setup flow shows
**Still connecting** instead of opening the app. Select **Reload** to try again.

If Gentic2 cannot read your saved settings, it shows **Could not read settings**.
Select **Retry** after storage becomes available. Setup does not replace
unreadable settings with defaults.

## Check your agents

Gentic2 checks each selected computer for Claude Code and Codex. If an agent is
...
Gentic2 finds directories that Claude Code or Codex has used. Git repositories
...
A large or malformed history can reach the scan limit. Gentic2 keeps the
...
30 days. You can continue those conversations in Gentic2.

Conversation import is best effort. Gentic2 keeps the first user prompt and the
...
conversation so Gentic2 does not remove user text. It reads one conversation at
