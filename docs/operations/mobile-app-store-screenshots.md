...
The screenshot harness runs the real mobile application against three disposable local G2
...
the software keyboard, and supplies deterministic Gentic2 Connect discovery rows to the real
...
1. Creates three temporary G2 base directories and starts a local server for each on an available
   port.
2. Creates Gentic2, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic Gentic2 review diff.
...
Every configured device defaults to dark appearance and the `gentic2` palette, so plain
...
Pass `--theme <id>` (repeatable) or `--theme all` to capture the app's other palettes: `gentic2`,
...
| ------------------------------------- | ---------------------------- | ----------------- | ----------------------------------------- |
| `apple/iphone-6.9/dark/gentic2/`      | disposable iPhone 17 Pro Max | 1320×2868         | App Store Connect iPhone 6.9-inch         |
| `apple/iphone-6.5/dark/gentic2/`      | disposable iPhone 14 Plus    | 1284×2778         | App Store Connect iPhone 6.5-inch         |
| `apple/ipad-13/dark/gentic2/`         | iPad Pro 13-inch (M5)        | 2752×2064         | App Store Connect iPad 13-inch, landscape |
| `google-play/phone/dark/gentic2/`     | Pixel AVD at 420 dpi         | 1080×1920         | Google Play phone, portrait 9:16          |
| `google-play/tablet-7/dark/gentic2/`  | Pixel AVD at 600dp width     | 1080×1920         | Google Play 7-inch tablet, portrait 9:16  |
| `google-play/tablet-10/dark/gentic2/` | Pixel AVD at 800dp width     | 1440×2560         | Google Play 10-inch tablet, portrait 9:16 |
...
The generated tree is deliberately aligned with the store upload fields:
    ...
    ├── apple/
    │   ├── iphone-6.9/dark/gentic2/{thread,terminal,review,threads,environments,agent-activity}.png
    │   ├── iphone-6.5/dark/gentic2/{thread,terminal,review,threads,environments,agent-activity}.png
    │   └── ipad-13/dark/gentic2/{thread,terminal,review,threads,environments}.png
    └── google-play/
        ├── phone/dark/gentic2/{thread,terminal,review,threads,environments,agent-activity}.png
        ├── tablet-7/dark/gentic2/{thread,terminal,review,threads,environments,agent-activity}.png
        └── tablet-10/dark/gentic2/{thread,terminal,review,threads,environments,agent-activity}.png

A light-only run writes the same tree under `light/`; `--appearance both` writes both appearance
folders, and each requested theme adds a sibling folder next to `gentic2/`.
...
timeout from 60 to 300 minutes). The default dispatch captures both appearances of the `gentic2`
...
ARM64 by default for local Apple Silicon development; CI sets `G2_SHOWCASE_ANDROID_ABI=x86_64` so the
...
`pnpm --filter @gentic2/mobile showcase` starts Expo on its normal port, so it is not compatible with
...
The Pending rows use the production offline outbox and point at the real Gentic2 and React fixture
