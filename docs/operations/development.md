...
Flags go directly after the task name, for example `vp run dev --home-dir /tmp/gentic2-dev`.
Add `--browser` to open a browser automatically.

### State and ports

Linked worktrees default to their own `.g2/userdata`, even when `GENTIC2_HOME` is set.
The main checkout defaults to `~/.g2/dev/userdata`. An explicit `--home-dir` wins in both cases.
Never run a development server against the live `~/.g2/userdata`.
...
but occupied ports can shift them. `GENTIC2_PORT_OFFSET` or `GENTIC2_DEV_INSTANCE` can select a
...
`GENTIC2_BUNDLED_DEV=0` opts out when debugging bundler differences. Two reload traps matter
...
including services unrelated to Gentic2. If you run untrusted services on that hostname, keep
...
```dotenv
GENTIC2_DEV_AUTH_TOKEN=<the value generated above>
```

The `g2.json` Setup Worktree action links that file to each worktree's `.env`. The dev runner reads repository env files at startup. `.env.local` and inherited process
...
```sh
export GENTIC2_DEV_AUTH_TOKEN="<the value generated above>"
```
