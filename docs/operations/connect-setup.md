# Gentic2 Connect setup

Deployment and client configuration for Gentic2 Connect. The [architecture note](../internals/g2-connect.md)
explains the trust boundaries; the [relay README](../../infra/relay/README.md#deployment) owns relay
provisioning instructions.

## Public application configuration

Gentic2 Connect is disabled in a fresh clone. To build against the production deployment, copy the
...
```dotenv
GENTIC2_CLERK_PUBLISHABLE_KEY=<publishable key>
GENTIC2_CLERK_JWT_TEMPLATE=<JWT template name>
GENTIC2_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
GENTIC2_RELAY_URL=https://relay.example.com
```
...
1. Create a public OAuth application for the G2 CLI, using authorization-code exchange with PKCE.
...
5. Set `GENTIC2_CLERK_CLI_OAUTH_CLIENT_ID` to the generated public client ID in local and release
   build environments.

## JWT template

Create a Clerk JWT template named `g2-relay` with claims:

```json
{ "aud": "gentic2-relay" }
```

Set `GENTIC2_CLERK_JWT_TEMPLATE=g2-relay` for clients and
`CLERK_JWT_AUDIENCE=gentic2-relay` for the relay. The production relay deployment environment
...
Development uses `gentic2-dev://app`; production uses `gentic2://app`. Update the array with
...
| Variant     | Callback                                       |
| ----------- | ---------------------------------------------- |
| Development | `clerk://com.gentic2.gentic2.dev.callback`     |
| Preview     | `clerk://com.gentic2.gentic2.preview.callback` |
| Production  | `clerk://com.gentic2.gentic2.callback`         |

Preserve existing entries. These callbacks are separate from the `gentic2-dev` / `gentic2-preview` / `gentic2` navigation schemes. A private development build using the production Clerk key still needs its development callback allowed by that instance's administrator; rebuilding the same package does not change the allowlist.

## Desktop passkeys

For a production macOS app with bundle ID `com.gentic2.gentic2`:
...
4. Check `https://<frontend-api>/.well-known/apple-app-site-association`. Its
   `webcredentials.apps` must include `<TEAM_ID>.com.gentic2.gentic2`.
...
GENTIC2_MACOS_PROVISIONING_PROFILE=/absolute/path/to/gentic2.provisionprofile
# Override only when the RP domain differs from the Clerk Frontend API hostname.
GENTIC2_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```
...
GENTIC2_PORT=13773 \
  "/Applications/Gentic2 (Alpha).app/Contents/MacOS/Gentic2 (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/Gentic2 (Alpha).app"
codesign -d --entitlements :- "/Applications/Gentic2 (Alpha).app"
```
