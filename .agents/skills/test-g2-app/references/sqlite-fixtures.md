...
Load this reference only when inspecting or seeding local G2 state directly.
...
```bash
node apps/server/scripts/g2-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/g2-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/g2-seed.sql
```
