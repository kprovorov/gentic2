---
name: test-g2-app
description: Test Gentic2's web and desktop UI through its built-in Browser panel against isolated development state. Use for browser verification, browser pairing recovery, and test fixtures. Use test-g2-mobile for native mobile verification.
---

# Test G2 web and desktop

Use G2's built-in Browser panel for verification. If its tools are absent or
...
testing, use [test-g2-mobile](../test-g2-mobile/SKILL.md).
...
`.g2` state and read the actual ports and pairing URL from the dev-runner output.
Never run against `~/.g2/userdata` or set `VITE_HTTP_URL` or `VITE_WS_URL`.
...
`preview_navigate`, then use `preview_snapshot` and G2's interaction tools.
