...
if ! pairing_output="$({
  GENTIC2_PORT="$server_port" node apps/server/src/bin.ts auth pairing create \
    --base-dir "$base_dir" \
...
process.stdout.write(`gentic2-dev://connections/new?${query}`);
NODE
)"

if ! "$agent_device_command" open com.gentic2.gentic2.dev "$deep_link" "$@" \
  >/dev/null 2>&1; then
