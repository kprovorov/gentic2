---
name: test-g2-mobile
description: Test Gentic2's native iOS and Android app through its Device panel and returned AgentDevice command. Use for mobile verification, native-client builds, Metro launch, and mobile pairing against isolated development state.
---
...
G2 boots the device and shows its live stream in the Device panel. Follow its
...
If G2 device tools or the selected device are unavailable, report the blocker
...
from the dev-runner output. Use the worktree's ignored `.g2` state. Never run
against `~/.g2/userdata`. The Browser panel is not required for this workflow.

Test with meaningful project and thread data. Read the shared
[SQLite fixture reference](../test-g2-app/references/sqlite-fixtures.md) only
...
`open com.gentic2.gentic2.dev <url>` and all returned target arguments.
...
```bash
.agents/skills/test-g2-mobile/scripts/pair-client.sh \
  <server-port> <base-dir> <device-reachable-backend-origin> \
  "$agent_device_command" "${agent_device_target_args[@]}"
```

It issues a fresh credential and opens Gentic2 Dev's existing pairing route
